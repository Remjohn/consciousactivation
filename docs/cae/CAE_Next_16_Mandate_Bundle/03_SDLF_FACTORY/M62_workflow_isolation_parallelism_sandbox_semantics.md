# Mandate 62 — Workflow Isolation, Parallelism + Sandbox Semantics
    ...
    Make parallel Agent/code execution safe and attributable by defining worktree, sandbox, fan-out, join, cancellation and cleanup semantics.
...
- `infra/`
    ...
    ### Allowed work
    Extend existing concurrency controls into workflow-level isolation semantics. Support read-only parallelism immediately; permit write-parallelism only through declared isolated workspaces/worktrees/sandboxes and deterministic join/race rules.

    ### Required artifacts
    Isolation policy; sandbox adapter contract; parallel workflow fixture; conflict tests; artifact attribution tests; CURRENT update.
...
assert all(isolated(node) for node in batch)
    ```

    ### SSSF reference example
    ```text
    The transcript’s progression from worktrees to isolated sandboxes is the reference pattern. The repo explicitly says SSSF leaves sandboxing/merge for later; CAE needs those stronger semantics because it is targeting controlled production.
    ```
...
- do not promote Agent-generated lessons into binding procedure without versioned validation through existing authority.
    ...
    - Parallel branches cannot mutate the same isolated target without an approved join strategy.
- Cancellation propagates deterministically.
- Artifacts are attributable to branch and Agent.
- Cleanup is idempotent/reconcilable.
- Race/first-success behavior has explicit termination semantics.
    ...
    Sandbox creation must be reversible; leaked external resources are cleaned or receipted. No destructive cleanup of unrelated operator workspaces.
    ...
    Run two write branches against the same path; kill one branch mid-write; return first-success while slower branches continue mutating the target; attempt to read another tenant’s sandbox.
