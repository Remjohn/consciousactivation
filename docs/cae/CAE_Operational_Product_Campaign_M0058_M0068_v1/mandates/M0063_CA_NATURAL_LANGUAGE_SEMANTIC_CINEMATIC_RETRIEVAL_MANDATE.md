# CAE-M0063 — Natural-Language Semantic Cinematic Retrieval
...
Implement natural-language retrieval over the governed scene corpus so semantic paraphrases can retrieve useful E/D-roll/B-roll/movie candidates while respecting hard source, role, rights and workspace constraints.
...
- M0061
- M0062 interface/index contract
- M10 Asset Intelligence
- current F04 retrieval implementation
- approved open model/model registry
- workspace isolation
...
Extend existing lexical retrieval with governed semantic retrieval. Return ranked candidates with exact source/timestamp, contextual explanation, semantic role, insert role, rights and provenance. Allow abstention.
...
Outputs:
Retrieval engine; embedding model binding; hybrid ranking; hard-negative fixtures; evaluation report; retrieval receipt.
...
No automatic rights clearance, no generated movie scenes, no closed model authority, no semantic mutation of AssetAnnotation.
...
Test paraphrase, exact terminology, role-filtered, rights-filtered and no-good-match cases using real indexed fixtures. Hard-negative set required.
...
False proof: cosine-nearest scene is semantically close but narratively wrong. It must be rejected by evaluation or ranking constraints.
...
`RETRIEVAL_REQUESTED → CANDIDATES → RANKED|ABSTAIN|BLOCKED`.
...
Disable new semantic path while preserving lexical fallback.

Failed evidence SHALL be preserved. Never delete a failed receipt to manufacture a clean result.

## 12. Operator Decision

Do you accept M0063 and authorize M0064?
...
Execute only CAE-M0063 — Natural-Language Semantic Cinematic Retrieval.

Authority:
M0061; M0062 interface/index contract; M10 Asset Intelligence; current F04 retrieval implementation; approved open model/model registry; workspace isolation
