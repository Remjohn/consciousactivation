# CAE Mandate M012 — Sovereign Source Media Byte Supremacy

**Mandate ID:** `CA-M012`  
**Wave:** `02`  
**Canonical question:** `Q12`  
**Causal stage:** `Stage 07 — Evidence Capture`  
**Governing requirement / invariant:** `FR-012 / sovereign media byte supremacy`  
...
Establish original source media bytes as the sovereign evidence source for downstream transcription, diarization, extraction, and semantic observation. Every derivative media or transcript artifact must retain immutable lineage to the exact source package/media identity and digest. A derivative can be useful and still remain derivative; it cannot silently replace the source bytes as authority. The architectural chain is SOURCE MEDIA BYTES → SOURCE IDENTITY/HASH → DERIVATIVE PROCESSING → EVIDENCE.
...
- `docs/cae/cae_master_57_question_convergence_canon.md` — `Q12`
- `docs/cae/UI.md`
- `docs/cae/Architecture.md`

**Question / product authority**
- `Architecture.md` Section 14 and evidence/provenance sections.\n- `UI.md` Section 13.\n- `PRD-003: Evidence Capture, Grounding & Yield Analysis`.\n- Wave 02 readiness assessment: CA-M012 / Q12.\n- Existing source package/storage, media hashing, derivative, and evidence-ingestion surfaces.

The Wave 02 readiness assessment supplied with this execution context explicitly defines CAE Wave 02 as Q09–Q16 and names this mandate as `CA-M012` / `Q12`. That planning statement is a source of intended scope; the current repository remains the source of executable truth.
...
- current source media storage/package model;\n- immutable media identity/hash helpers;\n- transcript/diarization/derivative models;\n- media ingestion path in `services/pipeline/src/cmf_pipeline/application.py` or its current equivalent;\n- evidence schema/migrations;\n- direct source-provenance tests.
...
Own source-media identity, digest, derivative linkage, and source-authority/mismatch checks. Reuse the current storage and hashing primitives. Every derivative consumed downstream must identify the exact source identity/digest from which it was produced. Provide operator inspection that distinguishes sovereign source from derivative. The scope ends at source/derivative integrity; temporal anchoring, continuity, verbatim rules, and Collision formation are later mandates.
...
- Do not treat a transcript, summary, proxy, or model extraction as the source.\n- Do not overwrite original bytes to repair metadata.\n- Do not invent a second media registry if one already exists.\n- Do not equate filename or URL with content identity.\n- Do not suppress source mismatch to make a pipeline green.\n- Do not implement temporal anchoring or quote semantics beyond minimum source linkage.
...
1. Inventory source package identity, storage, digests, derivatives, and evidence consumers. 2. Establish/reuse immutable source identity and digest. 3. Bind each derivative to exact source identity/digest and processing revision where present. 4. Add source-integrity verification before downstream evidence consumption. 5. Expose source-vs-derivative provenance. 6. Add positive tests for stable identity, lineage, retrieval, and downstream provenance. 7. Add negative tests for changed source bytes, digest mismatch, derivative without source, proxy substitution, and stale derivative binding. 8. Preserve historical source references when a package is superseded. 9. Record environment limitations.
...
The key proof is actual byte/hash lineage, not a metadata field that merely says “source.” What is measured: content identity, source/derivative linkage, and mismatch detection. What is not measured: transcript semantic accuracy, speaker correctness, or creative value. False-proof case: an internally consistent transcript whose source file changed after ingestion; it must become stale/untrusted rather than automatically valid. Environment fidelity requires the actual storage/hash path or a documented limitation.
...
Stop before Q13 temporal anchoring. The handoff artifact must provide a stable sovereign source identity/digest for all subsequent evidence objects. Request CA-M012 approval before progressing.

## 11. Rollback / recovery

If source bytes change, create a new source revision/package; do not edit the old identity. Derivatives tied to the old digest become stale or blocked. Preserve historical provenance and auditability.
...
**Operator decision required:** Approve CA-M012 and authorize CA-M013; confirm that source identity/digest is accepted as the sovereign evidence root.
...
```text
Execute `CA-M012` only. Read the Mandate Authoring Protocol, Gemini execution skill, Master Canon Q12, `Architecture.md` Section 14, `UI.md` Section 13, Wave 02 readiness assessment, PRD-003 evidence material, and the current source-media storage, hashing, derivative, ingestion, and test surfaces. Establish the original media bytes as sovereign. Every transcript, diarization result, model observation, and other derivative must retain exact source identity and digest lineage. Verify the actual byte/hash path and downstream provenance. Add negative tests for source mutation, digest mismatch, derivative without source linkage, proxy substitution, and stale binding. The false-proof case is a perfect-looking transcript whose underlying media no longer matches its registered digest; it must not become authoritative. Do not implement temporal anchoring, continuity, or verbatim capture beyond minimum source linkage. Record commands, environment, fixture/source identity, limitations, control state, and exact commit SHA. Request the CA-M012 operator decision and stop. Preserve the distinction between fact, hypothesis, and operator decision. Do not convert a green test into proof of a wider property. If a required dependency is unavailable or a higher-order invariant conflicts with the proposed change, record the collision, classify it, update control state, request the appropriate operator decision, and stop rather than inventing a substitute. Never claim completion from documentation alone; every completion claim must point to executable evidence and its limitations.
```
