# CAE Mandate M010 — Structured, Digest-Pinned Research Brief

**Mandate ID:** `CA-M010`  
**Wave:** `02`  
**Canonical question:** `Q10`  
**Causal stage:** `Stage 02 — Research & Evidence`  
**Governing requirement / invariant:** `FR-010 / structured causal research brief`  
...
Implement the Research Brief as a typed, provenance-bearing causal input rather than an opaque Markdown or prose blob. Each material research claim must have stable claim identity and be connected to source/citation identity, source digest or immutable source locator, authority tier, falsification condition, and the Research Brief revision containing it. The runtime must be able to validate the structure before downstream consumption, and a downstream program must receive the governed structured artifact rather than infer its meaning from prose. This mandate is about representation, provenance, revision, and admission, not about inventing a research methodology or deciding which external source is philosophically “best.”
...
- `docs/cae/cae_master_57_question_convergence_canon.md` — `Q10`
- `docs/cae/UI.md`
- `docs/cae/Architecture.md`

**Question / product authority**
- `UI.md` research/audience inspection guidance and the operator evidence model.\n- `Architecture.md` Sections 4, 5, 7, and 20.\n- `PRD-001: Audience & Research Intelligence`.\n- Wave 02 readiness assessment: CA-M010 / Q10.\n- Existing research models, manifests, provenance/digest utilities, APIs, and tests.

The Wave 02 readiness assessment supplied with this execution context explicitly defines CAE Wave 02 as Q09–Q16 and names this mandate as `CA-M010` / `Q10`. That planning statement is a source of intended scope; the current repository remains the source of executable truth.
...
- current Research Brief and research-claim types;\n- source/citation storage and provenance utilities;\n- audience/research program manifests;\n- downstream consumer inputs;\n- current schema/migrations and validation;\n- direct tests for research creation/consumption;\n- any existing FR-010 implementation or validator.
...
Own the typed Research Brief object and its direct provenance/validation contract. Reuse existing identifiers, digest functions, revision semantics, and source-reference models before creating alternatives. The minimum contract must permit a consumer to answer: what claim is this, which source supports it, which exact source revision/digest was used, what authority tier applies, how could the claim be falsified, and which brief revision contains it? Provide an operator-inspection path for provenance and blocked reasons. The mandate ends before new collision discovery, narrative generation, or audience-layer convergence.
...
- Do not build a web-research crawler or new research engine.\n- Do not treat a URL or clickable citation as sufficient provenance.\n- Do not allow unstructured prose to satisfy the typed contract by default.\n- Do not silently substitute a different source when a digest fails.\n- Do not add a score and present it as proof of research truth.\n- Do not mutate a previously sealed research revision in place.\n- Do not expand into Q01/Q02 convergence or Q16 Collision generation.
...
1. Inventory current research artifacts and consumers. 2. Define/reuse the smallest typed claim/brief schema. 3. Implement source identity/digest or immutable locator, authority tier, falsification condition, and revision linkage. 4. Establish deterministic serialization/digest behavior or reuse the repository primitive. 5. Enforce admission at the canonical API/runtime/program boundary. 6. Expose operator inspection of provenance and block reasons. 7. Add positive end-to-end tests for valid multi-claim briefs and revision preservation. 8. Add negative tests for missing citation, missing/bad digest where required, invalid authority tier, missing falsification condition, malformed claims, stale brief, and source substitution. 9. Record which checks are mechanical and which remain operator judgments.
...
The verifier must prove that the structured Research Brief is what downstream consumers actually read. A schema-only test is insufficient if the live consumer still receives prose. What is measured: structural completeness, source identity, digest/reference integrity, revision binding, and runtime admission. What is not measured: whether an external source is substantively true in the world or whether the research is creatively insightful. False-proof case: a polished brief with clickable citations but no verifiable source digest or immutable source identity. It must fail admission. Environment fidelity requires the real persistence/read/consumer path or a documented limitation.
...
Do not implement downstream collision semantics or new research discovery. Stop once a structured, revisioned Research Brief can be created, inspected, validated, and consumed through the canonical boundary with positive and negative evidence. Request the operator decision for CA-M010.

## 11. Rollback / recovery

Invalid briefs are rejected, superseded, or versioned rather than silently rewritten. Existing legacy artifacts that cannot satisfy the new contract must be classified and either migrated deterministically within scope or blocked. Do not fabricate provenance metadata. Preserve historical source references needed for audit.
...
**Operator decision required:** Approve CA-M010 and authorize its use as an input to CA-M011; record any operator decision required for legacy research artifacts.
...
```text
Execute `CA-M010` only. Read the Mandate Authoring Protocol, Gemini execution skill, Master Canon Q10, `UI.md`, `Architecture.md`, Wave 02 readiness assessment, PRD-001 research material, and the current research models, provenance utilities, manifests, API, and tests. Implement a structured Research Brief with stable claim identity, source/citation identity, source digest or immutable locator, authority tier, falsification condition, and revision identity. Make the runtime consume this structured artifact, not an opaque prose blob. Do not invent a research methodology or downstream Collision logic. Prove a valid multi-claim brief end to end and preserve prior revisions. Reject absent citations, bad or missing digests where required, invalid authority tiers, missing falsification conditions, malformed claims, stale revisions, and source substitution. The false-proof case is polished research with unverifiable source identity; it must fail. Record commands, environment, fixtures, actual property proved, limitations, control state, and exact commit SHA. Request the CA-M010 operator decision and stop. Preserve the distinction between fact, hypothesis, and operator decision. Do not convert a green test into proof of a wider property. If a required dependency is unavailable or a higher-order invariant conflicts with the proposed change, record the collision, classify it, update control state, request the appropriate operator decision, and stop rather than inventing a substitute. Never claim completion from documentation alone; every completion claim must point to executable evidence and its limitations.
```
