# CAE Mandate CA-M053 — CSEB Golden Benchmark Certification and Signed Model Routing Gates

**Status:** Proposed governed execution mandate for Wave 07.  
**Canonical decision:** Q52  
**Mandate ID:** `CA-M053`  
**Governing invariant:** `INV-BENCH-001`  
**Primary implementation surface:** `tests/test_model_benchmarks.py; packages/ca_runtime/src/ca_runtime/agent_invocation.py (benchmark evaluation and model certification routing)`
...
The canonical decision is Q52; the execution identifier is CA-M053. The primary invariant is `INV-BENCH-001`. This mandate consumes earlier decisions but does not reopen them. Any apparent collision with an earlier invariant must be treated as an authority conflict and escalated through the protocol’s collision procedure rather than resolved by convenience.

## 2. Decision / objective being authorized

The authorized objective is:

> **Establish the CAE Semantic/Execution Benchmark (CSEB) as a repeatable certification gate spanning semantic, governance, operational, economic, and human dimensions, with signed ModelCertificationReceipt tokens required before governed live routing.**
...
**Objective:** Establish the CAE Semantic/Execution Benchmark (CSEB) as a repeatable certification gate spanning semantic, governance, operational, economic, and human dimensions, with signed ModelCertificationReceipt tokens required before governed live routing.

**Dependencies:** Q38 resilient routing, Q39 structured repair, Q50 economics, Q51 Voice DNA, Q56 live proof harness.

**Allowed runtime/code surface:** `tests/test_model_benchmarks.py; packages/ca_runtime/src/ca_runtime/agent_invocation.py (benchmark evaluation and model certification routing)`
...
**Outputs:** CSEB benchmark cases, scoring contract, signed ModelCertificationReceipt schema/fixtures, routing gate, certification expiry/revocation handling, positive and negative tests.
...
For this mandate, the principal surface is `tests/test_model_benchmarks.py; packages/ca_runtime/src/ca_runtime/agent_invocation.py (benchmark evaluation and model certification routing)`. The intended artifact class is `CSEB benchmark cases, scoring contract, signed ModelCertificationReceipt schema/fixtures, routing gate, certification expiry/revocation handling, positive and negative tests.`. No change to unrelated UI surfaces, upstream semantic definitions, external integrations, deployment topology, or later-wave/question contracts is authorized unless a required dependency inside the declared surface makes the change unavoidable. In that case, stop and request an Operator collision decision rather than broadening scope unilaterally.
...
The mandate explicitly prohibits the following: Do not accept placeholder or dummy hashes, certify on a single score, substitute a static allowlist for current benchmark evidence, or treat benchmark execution in an isolated environment as proof of production routing compatibility.
...
For this mandate, the specific required behavior is: Define benchmark dimensions and minimum predicates; execute golden cases deterministically; generate signed certification receipts tied to model identity/version and benchmark revision; require valid receipts at routing time; reject expired, forged, placeholder, or mismatched certifications.

The executor must include a contrastive negative test for the following false-proof case:

> A model has a perfect semantic benchmark score and an apparently valid 64-character hash, so routing succeeds even though the hash is a dummy placeholder and the model lacks the current signed certification receipt required by policy.
...
Execute CA-M053 (Q52) only. Read the authoring protocol, Gemini skill, FR-052, convergence decision ledger, Architecture.md, UI.md, readiness assessment, and prior routing/economics/voice mandates. Objective: enforce CSEB golden benchmark certification across semantic, governance, operational, economic, and human dimensions, with a valid signed ModelCertificationReceipt required before governed live routing. Work only in test_model_benchmarks.py, agent_invocation.py, directly required certification schema/fixtures, and evidence artifacts. Meaning authority is the ratified Q52 decision; runtime authority is the canonical routing gate and certification receipt; Operator controls promotion. Do not accept dummy hashes, single-score certification, static allowlists as substitutes for current certification, or isolated benchmark runs as proof of production routing compatibility. Define benchmark cases and minimum predicates; issue receipts bound to model identity/version and benchmark revision; reject forged, expired, placeholder, or mismatched certifications. Include the false-proof case where an apparently valid 64-character hash is merely a dummy and no signed receipt exists. Record evidence classes, positive/negative tests, limitations, environment fidelity, control-state update, and exact commit SHA. Stop on authority collision, missing cryptographic evidence, unsafe schema changes, or adjacent redesign. Completion requires routing denial without valid certification and successful routing only with valid certification. Request Operator decision: APPROVE, REJECT with findings, or DEFER with named remediation. Do not execute Q57.
