# CAE Mandate CA-M057 — Live End-to-End Execution Proof Harness and Replay-Exact Certification

**Status:** Proposed governed execution mandate for Wave 07.  
**Canonical decision:** Q56  
**Mandate ID:** `CA-M057`  
**Governing invariant:** `INV-LIVE-001`  
**Primary implementation surface:** `packages/ca_runtime/src/ca_runtime/program_operator_runtime.py; api/routers/programs.py (LiveProgramExecutionHarness, CLI/API /run --live, gate pause, receipts, replay)`
...
The canonical decision is Q56; the execution identifier is CA-M057. The primary invariant is `INV-LIVE-001`. This mandate consumes earlier decisions but does not reopen them. Any apparent collision with an earlier invariant must be treated as an authority conflict and escalated through the protocol’s collision procedure rather than resolved by convenience.

## 2. Decision / objective being authorized

The authorized objective is:

> **Prove the CAE runtime can execute a real end-to-end program through worker lease acquisition, live model inference, human gate suspension/approval, durable Merkle receipts, and bit-for-bit persisted replay with `is_synthetic: false`.**
...
**Objective:** Prove the CAE runtime can execute a real end-to-end program through worker lease acquisition, live model inference, human gate suspension/approval, durable Merkle receipts, and bit-for-bit persisted replay with `is_synthetic: false`.

**Dependencies:** Q41 CAS, Q42 Merkle chaining, Q43 replay, Q44 reconciliation, Q45 preemption, Q46 fencing, Q47 sandbox, Q48 registry pinning, Q50 economics, Q52 certification, Q55 WAL.

**Allowed runtime/code surface:** `packages/ca_runtime/src/ca_runtime/program_operator_runtime.py; api/routers/programs.py (LiveProgramExecutionHarness, CLI/API /run --live, gate pause, receipts, replay)`
...
**Outputs:** LiveProgramExecutionHarness, CLI/API live-run wiring, end-to-end fixture or safe test program, live execution receipts, replay parity report, proof manifest, environment-fidelity record.
...
For this mandate, the principal surface is `packages/ca_runtime/src/ca_runtime/program_operator_runtime.py; api/routers/programs.py (LiveProgramExecutionHarness, CLI/API /run --live, gate pause, receipts, replay)`. The intended artifact class is `LiveProgramExecutionHarness, CLI/API live-run wiring, end-to-end fixture or safe test program, live execution receipts, replay parity report, proof manifest, environment-fidelity record.`. No change to unrelated UI surfaces, upstream semantic definitions, external integrations, deployment topology, or later-wave/question contracts is authorized unless a required dependency inside the declared surface makes the change unavoidable. In that case, stop and request an Operator collision decision rather than broadening scope unilaterally.
...
The mandate explicitly prohibits the following: Do not substitute aggregate-row initialization for execution, use mocks where real lease/model/tool boundaries are required, mark synthetic runs as real, skip the human gate, or claim replay parity from reconstructed in-memory state.
...
For this mandate, the specific required behavior is: Execute a controlled live program through the actual public boundary; acquire a real lease; perform real inference under economic and certification gates; pause at the declared human gate; resume only after valid authorization; persist receipts; reopen durable history and replay; compare canonical outputs bit-for-bit; record any environmental limitation.

The executor must include a contrastive negative test for the following false-proof case:

> The CLI returns a successful run ID and the final aggregate is populated, but no worker lease was acquired and inference was never performed. The test passes because it inspects database state rather than proving physical live execution.
...
Every substantive result shall be tagged using one or more evidence classes: `EXECUTABLE, TEST, SCHEMA, DOCUMENT, OPERATOR_DECISION_REQUIRED`. The verifier must establish the intended property, not merely a correlated symptom.
...
Execute CA-M057 (Q56) only. Read the authoring protocol, Gemini skill, FR-056, convergence decision ledger, Architecture.md, UI.md, readiness assessment, and all predecessor mandates Q41–Q55 that materially constrain live execution. Objective: prove the real end-to-end CAE path through worker lease acquisition, live model inference, human gate suspension/approval, durable Merkle receipts, and bit-for-bit persisted replay with is_synthetic=false. Work only in program_operator_runtime.py, api/routers/programs.py, directly required live-harness/tests/evidence artifacts. Meaning authority is Q56 plus prior ratified invariants; runtime authority is the canonical public boundary and persisted state/receipt paths; Operator controls approval. Do not substitute aggregate-row initialization, mocks at material boundaries, synthetic labels, skipped human gates, or in-memory replay for live proof. Execute a controlled real run through CLI/API, verify a real lease and inference, pause at the declared gate, resume only after authorization, persist receipts, reopen durable history, replay, and compare canonical outputs bit-for-bit. Include the false-proof case where a run ID and populated row exist but no lease or inference occurred. Record evidence classes, environment fidelity, limitations, control-state update, and exact commit SHA. Stop on failed dependency, unavailable live environment, authority collision, or any need to weaken Q41–Q55. Completion does not authorize Q57. Request Operator decision: APPROVE, REJECT with findings, or DEFER with named remediation, then stop.
