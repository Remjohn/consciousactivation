...
Implement the Campaign Content Portfolio Contract so deliverable quantities, target aspect ratios, and format requirements are frozen before physical evidence acquisition and remain the authoritative production target for downstream yield.
...
Canonical invariant: `FR-PORT-001`. The canon identifies `packages/ca_runtime/src/ca_runtime/collision_hypothesis_store.py#L30-L50` and the interview program's approved collision portfolio prerequisite as precheck surfaces.
...
- `docs/cae/cae_mandate_bundle/01_CA_MANDATE_AUTHORING_PROTOCOL.md`
- `docs/cae/cae_master_57_question_convergence_canon.md` — Q08
- `UI.md` — Sections 6, 9, 12, 18, 19
- `Architecture.md` — Sections 9, 10, 13, 17–19
- `docs/cae/CAE_Product_Brief/05_CAE_Causal_Operating_Model.md`
- `packages/ca_runtime/src/ca_runtime/collision_hypothesis_store.py`
- `programs/interview_semantic_program/program_manifest.yaml`
- current campaign creation/output-target/format-profile contracts and tests
...
The mandate owns the frozen portfolio contract and its relationship to interview/preproduction admission. It may normalize existing output target data into an immutable portfolio revision. It must capture at least deliverable identity/quantity, target format/aspect requirements, and the campaign/narrative context required to interpret the contract.

The contract must be frozen before evidence acquisition. Once evidence acquisition begins, downstream components may evaluate yield against the contract, but they must not silently alter the target to fit what was captured. If the operator needs a different portfolio, create a new governed revision before the relevant execution boundary.

The UI must distinguish draft portfolio, sealed portfolio, and active execution snapshot.
...
- Do not allow evidence yield to redefine the deliverable target after capture starts.
- Do not treat output-target UI state as the contract.
- Do not silently mutate a frozen portfolio because rendering failed.
- Do not implement yield gating itself beyond the minimum interface needed to bind the contract.
- Do not redesign campaign creation outside the portfolio fields required by this question.
...
1. Inventory campaign output targets, format profiles, collision portfolio storage, and interview preconditions.
2. Define the authoritative portfolio object/revision and required deliverable fields.
3. Implement draft → sealed/frozen lifecycle with clear execution binding.
4. Prevent mutation of a frozen portfolio in place.
5. Ensure interview/session initialization requires the correct approved portfolio revision.
6. Add positive tests for portfolio creation, sealing, and downstream retrieval.
7. Add negative tests for missing portfolio, mutation after freeze, invalid format requirements, and stale portfolio revision.
8. Add integration evidence from campaign setup through interview admission.
9. Update UI to show the frozen contract and its revision without creating a second source of truth.
...
Evidence must show the portfolio is a production contract, not a form: schema/model evidence for deliverables, quantities, aspect/format requirements, and revision; executable freeze/mutation tests; negative admission test proving interview/evidence acquisition cannot start without an approved portfolio; negative test proving a frozen contract cannot be changed in place; integration evidence showing downstream execution binds to the exact portfolio revision; and UI projection evidence.

False-proof countercase: storing output targets at campaign creation while allowing later API calls to overwrite them during an active run. That is not a frozen contract.
Environment fidelity: use the canonical campaign/runtime boundary for freeze and admission tests.
...
Stop once a sealed portfolio contract is required before physical evidence acquisition and its immutable revision is bound into the scoped execution path. If existing campaign creation lacks enough authoritative information to define the portfolio, stop at the missing contract and request an operator/product decision rather than guessing.
...
Use immutable revisions or additive contract records. Never rewrite an active execution's portfolio. If a portfolio change is needed, create a new revision and preserve the previous one for audit and rollback.
...
Approve or reject whether the campaign now has a durable, immutable content portfolio contract that is frozen before evidence acquisition and bound to execution.
...
> Execute `CA-M008` only. Read the protocol, Master Canon Q08, `UI.md`, `Architecture.md`, the causal operating model, current campaign/output-target code, collision hypothesis store, and interview semantic manifest. Implement `FR-PORT-001`: the Campaign Content Portfolio Contract must freeze deliverable quantities, aspect ratios, and format requirements before physical evidence acquisition. Treat the portfolio as an authoritative versioned production contract, not a mutable UI form. Require the correct approved portfolio revision at interview/session admission and reject mutation of a frozen contract in place. Add positive and negative executable tests plus an integration proof from campaign setup to interview admission. Do not implement full yield gating or redesign campaign creation. If required portfolio semantics are not authoritative in current sources, stop and report the missing decision. Finish with evidence locators, limitations, changed files, tests, commit SHA, and an explicit Operator approval/rejection request. Before making any edit, inspect the current implementation and identify the authoritative boundary you will change. Do not infer missing behavior from documentation alone. Preserve existing compatible behavior, keep the change minimal, and test the failure mode that would otherwise create a convincing but invalid result. Do not continue into the next mandate. If a required source of authority is absent or contradictory, stop and report the collision. The completion report must distinguish executable proof from documentation, list every changed file, include exact test commands and evidence locations, state residual limitations, capture the exact commit SHA, and explicitly ask the Operator whether to approve or reject this mandate.
