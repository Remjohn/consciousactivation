...
def rollback_from_receipt(repo,receipt,force=False):
    ...
    transient={"__pycache__",".pytest_cache",".mypy_cache",".ruff_cache","node_modules","phase9-demo-output"}
...
def main():
    ...
    plan={"schema_version":"ca-phase09-apply-plan/v1","bundle_id":bundle_id,"repo":str(repo),"actions":actions,"failures":failures,"result":"PASS" if not failures else "FAIL"}; print(json.dumps(plan,indent=2,sort_keys=True))
    if failures: return 2
    if args.dry_run: return 0
    for tool in ("node","npm","tsc","ffmpeg","ffprobe","git"):
        if shutil.which(tool) is None: raise SystemExit(f"Phase 9 validation requires {tool} on PATH")
    ...
    try:
        ...
        if not args.skip_tests:
            ...
            main_report=state/"validation_report.json"; install_report=state/"clean_install_report.json"; main_log=state/"validation.log"; install_log=state/"clean_install.log"; artifact_dir=state/"reference-artifacts"
            with install_log.open("w",encoding="utf-8") as log:
                install_check=subprocess.run([sys.executable,str(repo/"scripts"/"phase9"/"validate_phase9_clean_install.py"),"--report",str(install_report)],cwd=repo,text=True,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=3600)
            with main_log.open("w",encoding="utf-8") as log:
                main_check=subprocess.run([sys.executable,str(repo/"scripts"/"phase9"/"validate_phase9.py"),"--report",str(main_report),"--artifact-dir",str(artifact_dir)],cwd=repo,text=True,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=7200)
        ...
        if args.commit:
            ...
            git(repo,"add","--all"); git(repo,"commit","-m","implement Phase 9 final integrated development candidate"); receipt["git_commit_sha"]=git(repo,"rev-parse","HEAD").stdout.strip(); receipt_path.write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n")
        return 0
    except Exception:
