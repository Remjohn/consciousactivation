...
from .retrieval_engine import AuthorityFirstRetrievalService
from .skill_registry import SkillRegistry
from .programmed_model_engine import ProgrammedModelRegistry
from .cross_derivative import CrossDerivativeContinuityService
from .operations import PipelineOperationsService
...
class PipelineApplication:
    def __init__(self, database_path: str | Path | None = None):
        ...
        self.repairs = BoundedRepairService(self.repository)
        self.visual_delegation: VisualDelegationService | None = None
        self.skills = SkillRegistry(self.repository)
        self.retrieval = AuthorityFirstRetrievalService(self.repository)
        self.programmed_models = ProgrammedModelRegistry(self.repository)
        self.continuity = CrossDerivativeContinuityService(self.repository)
        self.operations = PipelineOperationsService(self.repository)
    ...
    def configure_visual_delegation(self, release_root: str | Path) -> VisualDelegationService:
        self.visual_delegation = VisualDelegationService(release_root)
        return self.visual_delegation
    ...
    def status(self) -> dict[str, Any]:
        return {
            **self.repository.health(),
            ...
            "phase3_core_implemented": True,
            "real_media_execution": True,
            "composition_runtime": True,
            "rendered_evaluation": True,
            "external_product_execution": self.visual_delegation is not None,
            "authority_first_retrieval": True,
            "skills_and_steering_recipes": True,
            "programmed_model_resolver": True,
            "cross_derivative_continuity": True,
