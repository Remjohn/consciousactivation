...
def get_execution_receipts(
    ...
    """Retrieves receipt evidence persisted or reconstructable from the canonical runtime ledger."""
    try:
...
def get_execution_receipt(
    ...
    """Retrieves one receipt by immutable receipt identifier."""
    try:
        receipts = service.get_execution_receipts(aggregate_id=aggregate_id, receipt_id=receipt_id)
        if not receipts:
            raise HTTPException(
...
def get_execution_failure(
    ...
    """Surfaces authoritative failure state and recovery-facing diagnostics."""
    try:
