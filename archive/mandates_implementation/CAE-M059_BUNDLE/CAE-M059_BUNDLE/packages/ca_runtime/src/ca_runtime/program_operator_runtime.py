...
class ProgramOperatorRuntimeService:
    ...
    def _control_receipt(
        *,
        ...
        """Builds a persisted control receipt without inventing a second state authority."""
        return {
            "receipt_id": receipt_id,
    ...
    def _project_receipts(self, aggregate_id: str) -> List[ExecutionReceiptProjection]:
        """Projects durable receipt evidence from the canonical aggregate and transition ledger."""
        ...
        # Phase-one registration and phase-two launch receipts are durable through the
        # canonical aggregate/lease records even though they are not transition records.
        ...
        if aggregate.version >= 1 and dispatch is not None:
            run_receipt_id = (
                f"rcpt_dispatch_{hashlib.sha256(f'{aggregate_id}:1:{dispatch.get("actor_id", "operator")}'.encode('utf-8')).hexdigest()[:24]}"
            )
            receipts.append(
                ExecutionReceiptProjection(
                    ...
                    program_version=aggregate.program_version,
                    operation="RUN",
                    actor_id=str(dispatch.get("actor_id") or "operator"),
                    lane=AuthorityLane.COMMANDER,
        ...
        # Preserve deterministic chronological ordering while keeping receipt IDs unique.
        deduped: Dict[str, ExecutionReceiptProjection] = {}
        for receipt in receipts:
            deduped[receipt.receipt_id] = receipt
    ...
    def get_execution_receipts(
        ...
        """Retrieves only receipts already evidenced by canonical runtime state/transition records."""
    ...
    def get_execution_failure(self, aggregate_id: str) -> ExecutionFailureProjection:
        """Surfaces failure only from the authoritative aggregate lifecycle and state payload."""
        aggregate = self.runtime.get_aggregate(aggregate_id)
    ...
    def pause_program(
        ...
        *,
        ...
        receipt_id = f"rcpt_pause_{hashlib.sha256(f'{aggregate_id}:{current.version + 1}'.encode('utf-8')).hexdigest()[:24]}"
        prior_receipts = list(current.state_data.get("operator_control_receipts", []))
        control_receipt = self._control_receipt(
            aggregate=current,
            receipt_id=receipt_id,
            operation="PAUSE",
            actor_id=actor_id,
        ...
        prior_receipts.append(control_receipt)
        updated = self.runtime.set_lifecycle(
            aggregate_id=aggregate_id,
        ...
        *,
        ...
        if current.lifecycle != ProgramStateLifecycle.PAUSED:
            raise ProgramTransitionBlockedError(
                aggregate_id=aggregate_id,
                transition_name="resume",
                reason=f"Cannot resume program from lifecycle state '{current.lifecycle.value}'. Expected 'PAUSED'",
            )
        receipt_id = f"rcpt_resume_{hashlib.sha256(f'{aggregate_id}:{current.version + 1}'.encode('utf-8')).hexdigest()[:24]}"
        prior_receipts = list(current.state_data.get("operator_control_receipts", []))
        control_receipt = self._control_receipt(
            aggregate=current,
            receipt_id=receipt_id,
