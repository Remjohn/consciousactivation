...
class IsolatedRuntimeContainer:
    ...
    def call(
        ...
        *,
        ...
        if len(request_bytes) > effective_quota:
            raise ByteQuotaExceededError(
                "external call request exceeded byte quota",
                reason_code="ERR_BYTE_QUOTA_INPUT",
                details={"bytes": len(request_bytes), "quota": effective_quota},
            )
    ...
    def _invoke_with_failover(
        ...
        *,
        ...
        for index, provider in enumerate(self.providers):
            ...
            except (ByteQuotaExceededError, InputSanitizationError):
                # Quota and sanitization are host invariant failures, not provider
                # availability failures, so they must never be hidden by failover.
                raise
