...
def test_inventory_only_consumes_approved_moments(tmp_path):
 ...
 phrase=p['object']['payload']['phrases'][-1];po=app.repository.store_object('packed_phrase',{'phrase_id':phrase['phrase_id'],'version':'1.0.0',**phrase},object_id=phrase['phrase_id'],idempotency_key='phrase',lifecycle_state='VALIDATED');phr=ref(po);span=make_source_span(source_ref=pref,start_ms=1500,end_ms=3240,speaker_id='guest')
 ...
 quote='Um then I learned to listen.'
 start=raw_transcript.index(quote)
 end=start+len(quote)
 media=app.repository.get_object(pref['object_id'])['payload']['media_assets'][0]
 evidence=app.verbatim.admit(
  source_package_ref=pref,alignment_ref=ar,phrase_refs=[phr],
  source_media_asset_id=media['asset_id'],source_media_sha256=media['sha256'],
  source_span=span,transcript_text=raw_transcript,
  transcript_sha256=hashlib.sha256(raw_transcript.encode('utf-8')).hexdigest(),
  character_start=start,character_end=end,quote_text=quote,
  actor_id='op',idempotency_key='inv:verbatim'
 )
 inv=app.inventory.compile(source_package_ref=pref,expression_moment_refs=[ref(approved)],phrase_pack_ref=pr,visual_index_ref=vr,reaction_receipt_refs=[],idempotency_key='inv2',verbatim_evidence_refs=[ref(evidence)])
