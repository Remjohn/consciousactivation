        ...
        with self._lock:
            ...
            if (
                ...
                raise ProgramLeaseConflictError(
                    aggregate_id=aggregate_id,
                    expected_lease_version=expected_lease_version,
                    actual_status=lease.get("status"),
                    actual_lease_version=lease.get("lease_version"),
                )
        ...
        *,
        ...
        with self._get_connection() as conn:
            try:
                ...
                if current_aggregate.version != expected_lease_version or current_aggregate.lifecycle != ProgramStateLifecycle.INITIALIZED:
                    raise ProgramLeaseConflictError(
                        aggregate_id=aggregate_id,
                        expected_lease_version=expected_lease_version,
                        actual_status=row["status"],
