# M0074 — Wind Comic Surgical Storyboard Production Extraction
...
The authorized decision is: **Extract and adapt only the production-storyboard behaviors from Wind Comic that improve shot planning, sketch-lock, timeline inspection, scene consistency, audit, and round-trip revision. Produce an isolated CAE-compatible reference implementation or adapter without importing provider-specific generation architecture.**
...
Extract only Wind Comic behavior relevant to storyboard/shot workshop, sketch-lock, shot timing/audit, scene consistency, timeline interaction, comments/revision and round-trip storyboard editing. The extraction must identify exact source files/symbols and produce a minimal isolated adapter/reference. Provider gateways, generation-provider abstractions and unrelated application domains are explicitly excluded.
...
**Allowed boundary:** `campaign extraction/reference artifacts, isolated `engines/storyboard/references/wind_comic/` only if justified, and focused tests`
...
No provider gateway adoption, no generation-provider dependence, and no wholesale repository copy.
