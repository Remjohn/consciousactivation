...
The authorized decision is: **Extract the minimum Jellyfish workspace behaviors needed for WYSIWYG storyboard editing, shot preparation, references, asset reuse, versioning, and visual inspection. Bind these behaviors to CAE StoryboardSession concepts rather than copying its domain model.**
...
Extract only Jellyfish behavior relevant to WYSIWYG storyboard editing, shot preparation, reference assets, candidate presentation, asset reuse, versioning and visual inspection. Map each adopted behavior into CAE StoryboardSession/Revision concepts. Do not adopt Jellyfish as semantic authority or copy its entire project model.
...
**Allowed boundary:** `campaign extraction/reference artifacts, isolated `engines/storyboard/references/jellyfish/` only if justified, and focused tests`
...
No Jellyfish domain authority, no replacement of EditorialStoryboard, and no wholesale repository copy.
