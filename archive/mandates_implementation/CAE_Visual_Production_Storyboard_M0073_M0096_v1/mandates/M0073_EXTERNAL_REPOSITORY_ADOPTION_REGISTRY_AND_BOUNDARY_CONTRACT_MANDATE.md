# M0073 — Visual Production Authority Pack Freeze, External Repository Adoption Registry and Boundary Contract
...
The authorized decision is: **freeze the Visual Production Authority Pack for this campaign and establish the authoritative external-repository adoption registry, recording exactly which capabilities are extracted from each upstream repository, which are not adopted, licensing constraints, boundary type, pinned source reference, and CAE-owned destination. No application source is merged in this mandate.**

The implementation must be narrow enough to test independently and explicit enough that a later integration agent can compose it without reverse-engineering the intent. The Authority Pack is supplied with this campaign bundle and is the canonical source for the visual-production updates; agents MUST NOT require those files to have already been copied into the CAE brownfield repository. Where a repository supplies useful behavior, the mandate adopts the behavior—not the repository's authority model. Where CAE already has a canonical object or service, this mandate must extend or adapt it rather than creating a duplicate.
...
Define the adoption registry plus validation tooling. Include Wind Comic, Jellyfish, DramaClaw, ArcReel, Seedance2 Storyboard Generator, WaooWaoo, Toonflow, OpenChatCut, Open Carrusel, Slidev, reveal.js, Rough Notation, chenglou/pretext, Skia, Meta SAM3 and sam3.cpp. Classify each as behavioral reference, isolated upstream dependency, embedded primitive, runtime, or visual-intelligence engine. Record URL, license, source commit/tag, exact adopted behavior, excluded behavior, CAE destination, process boundary and final integration owner. No runtime integration is permitted in M0073.
...
**Allowed boundary:** `docs/cae/, governance/program-control/, campaign control manifests, authority-pack manifests, and validation tooling`
...
No cloning, runtime launch, code merge, or architecture rewrite in the CAE repository. External source repositories may be inspected read-only as evidence for the adoption registry. The supplied Authority Pack is campaign input, not a request to copy all authority files into the CAE repository.
