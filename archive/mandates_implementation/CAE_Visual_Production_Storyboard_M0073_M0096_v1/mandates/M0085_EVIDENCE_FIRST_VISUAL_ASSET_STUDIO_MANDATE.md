# M0085 — Evidence-First Visual Asset Studio
...
The authorized decision is: **Build the operator-facing Visual Asset Studio foundation: source/evidence panel, composition canvas, layer inspector, transform controls, keyframe inspection, validation, revisions and governed linkage to existing VAE/Composition IR.**
...
Build the Visual Asset Studio with evidence/source panel, playable preview, composition canvas, layer inspector, transformation controls, keyframe inspection, validation, versioning, operator feedback and Visual Chat hooks. The Studio must operate on governed VAE/Composition IR and Storyboard objects rather than introducing a shadow state model.
...
**Allowed boundary:** `apps/web/, api/, services/ or packages/ directly supporting Visual Asset Studio, and tests`
