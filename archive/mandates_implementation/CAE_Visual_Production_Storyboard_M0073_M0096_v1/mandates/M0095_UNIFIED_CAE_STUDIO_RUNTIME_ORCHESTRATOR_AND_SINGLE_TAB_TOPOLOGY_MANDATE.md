# M0095 — Unified CAE Studio Runtime Orchestrator and Single-Tab Topology
...
**Campaign batch:** E1 RUNTIME INTEGRATION
...
The authorized decision is: **Implement the shared development/runtime orchestration layer that launches CAE API/UI, OpenChatCut, presentation runtimes, SuperVisual services and SAM3 behind one Studio origin/gateway with health checks, capability registration and isolated process boundaries.**
...
Implement one development/runtime orchestrator and one Studio origin. It must launch CAE API/UI, OpenChatCut, presentation runtime, SuperVisual service and SAM3 where enabled, expose health and capability registration, and surface launch failures. Runtime processes remain isolated and their internal authorities unchanged.
...
**Allowed boundary:** `deployment/dev/caestudio/, apps/web/, gateway/orchestrator packages/services, tests`
