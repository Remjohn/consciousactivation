# M0079 — Canonical Storyboard Session and Revision Domain

**Status:** EXECUTABLE — OPERATOR REVIEW REQUIRED — AUTHORITY-PACK INPUT REQUIRED  
...
**Campaign batch:** A0 FOUNDATION

## 1. Identity and status

This mandate is part of the first Visual Production / Evidence-First Storyboard campaign after the repository's M72 frontier. A prior attempted run may have stopped because the Visual Production Update documents were not physically present in the supplied brownfield archive; that prior no-write result does not satisfy this mandate. Execute this mandate against the supplied frozen Authority Pack and current brownfield repository, and only close after implementation evidence exists. It authorizes one bounded change and does not authorize architectural redesign outside its stated scope. The campaign is intended to make Storyboard, Visual Asset Editing, Transformation, Visual Intelligence and runtime handoff executable as one governed CAE production path.

## 2. Decision / objective being authorized

The authorized decision is: **Create the CAE-native StoryboardSession, StoryboardRevision, StoryboardScene, StoryboardShot, StoryboardElement, VisualAssetReference, TransformationIntent and related persisted domain objects as the canonical editable production workspace around existing EditorialStoryboard.**
...
The agent MUST read the campaign-supplied `AUTHORITY_PACK/` copies of the Visual Production Update documents before editing. Those files are mandatory campaign inputs and need not already exist in the CAE repository. The agent MUST also read:
...
Implement canonical editable StoryboardSession/Revision/Scene/Shot/Element/Asset/Transformation objects around existing EditorialStoryboard. Persist lineage to semantic program and harness, source evidence, transformation intent, validation, operator feedback and compile receipts. Extend existing objects where possible; do not create a parallel storyboard authority.
...
**Allowed boundary:** `packages/, services/, docs/cae/specs/, tests directly supporting storyboard domain`
...
Complete only when the requested artifact exists, tests pass, evidence is recorded, limitations are explicit, the exact commit SHA is known, and an operator decision is requested. A prior APPROVE-WITH-LIMITATIONS handoff with zero implementation is not a completion of M0079; the mandate remains executable under this corrected Authority-Pack input model. Stop when authority conflicts, required source or runtime evidence is unavailable, scope must widen, or acceptance cannot be established honestly. A blocker is a valid completion state when it is documented and routed for operator decision.
