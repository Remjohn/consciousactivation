...
**Campaign batch:** C1 OPERATOR STUDIO
...
The authorized decision is: **Implement a first-class AssetResearchSession that projects CAE Asset Intelligence retrieval into fast playable candidate discovery for real media, transcript-linked temporal excerpts, screenshots and evidence assets. PlayPhrase-like behavior is an adapter/presentation feature, not a competing retrieval authority.**
...
Implement AssetResearchSession as the operator projection of existing Asset Intelligence. Support transcript phrase retrieval, semantic/cinematic retrieval, source/time-range resolution, playable preview references, rights/provenance, and promotion requests. PlayPhrase-like functionality is a temporal retrieval/preview adapter and must not fork AuthorityFirstRetrievalService.
...
**Allowed boundary:** `services/asset-intelligence/, api/, apps/web/, packages/, tests directly supporting AssetResearchSession`
