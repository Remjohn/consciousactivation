# M0089 — Operator Visual Feedback and Teaching Loop
...
The authorized decision is: **Implement GOOD / NEEDS_EDIT / REJECT feedback with optional structured reason, note, region and revision references, and expose the resulting records to evaluation/harness datasets without silently changing production rules.**
...
Implement immutable GOOD/NEEDS_EDIT/REJECT feedback with optional reason, note, region and affected element/scene. Provide an evaluation projection that can train rankings or benchmarks later but does not silently rewrite standards. Include contrastive good-looking-but-wrong feedback examples.
...
**Allowed boundary:** `packages/, services/, evaluation/, tests`
