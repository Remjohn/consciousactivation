# M0094 — Native OpenChatCut Storyboard Handoff and Bidirectional Inspection
...
**Campaign batch:** D1 FORMAT RUNTIMES
...
The authorized decision is: **Bind resolved VideoStoryboard output to the native OpenChatCut runtime and preserve native timeline access, exact source/media hashes, storyboard lineage, operator edits and human-resolution persistence. Prove one-way compile and safe bidirectional inspection/update boundaries.**
...
Bind VideoStoryboardProgram output to native OpenChatCut through the existing CAE adapter boundary. Preserve exact source/media hashes, timeline lineage, operator edits and human-resolution records. Prove compilation into native timeline, native inspection, safe update/reconciliation and fail-closed behavior when source/runtime state diverges.
...
**Allowed boundary:** `engines/video/openchatcut/, adapter boundary, services/pipeline/, apps/web/, tests`
