...
TRANSFORMATION_MOTIONS = frozenset({
    "STATIC",
    "SUBTLE_ZOOM",
    "SUBTLE_PAN",
    "RETURN",
})
TRANSFORMATION_SOURCE_QUALITIES = frozenset({
    "HIGH", "MEDIUM", "LOW", "DEGRADED", "UNKNOWN",
})
TRANSFORMATION_SOURCE_ROLES = frozenset({
    "A_ROLL", "B_ROLL", "E_ROLL", "DOCUMENT", "PATTERN_INTERRUPT",
})


class TransformationIntent(BaseModel):
    """Typed reason for changing a source, upstream of executable recipe detail.

    ``source_element_id`` remains the canonical brownfield field.  ``source`` is
    exposed as a read-only alias so the model also satisfies the M0083 contract
    language without creating a second source authority.  Older M0079 payloads
    that used a narrative intent token in ``mode`` are normalized into the new
    explicit ``intent`` + ``mode=AUTO`` shape.
    """
    ...
    @property
    def source(self) -> str:
        """Canonical source alias for the M0083 projection contract."""
    ...
    def compile(
        ...
        *,
        ...
        """Compile through the bounded M0083 recipe registry."""
...
class TransformationRecipe(BaseModel):
    ...
    The fields added for M0083 are optional for M0079 brownfield compatibility;
    only registry-compiled recipes set ``governed=True`` and carry a digest.
    """
