...
class SourceQualityProfile(BaseModel):
    """Measured source-condition profile used to bound editorial transformations."""
    ...
    @field_validator(
        "crop_retention_bps",
        "compression_loss_bps",
        "sharpness_bps",
        "prior_degradation_bps",
    )
    @classmethod
    def bounded_basis_points(cls, value: int) -> int:
    ...
    def presentation_strategies(self) -> List[str]:
        ...
        return list(strategies[self.quality_level])


class TransformationValidationResult(BaseModel):
    """Deterministic quality gate result; contains no semantic interpretation."""
...
def _declared_transform_intensity(recipe: "TransformationRecipe") -> Dict[str, Any]:
    scale_deltas: List[int] = []
    reframe_bps: List[int] = []
    motion_bps: List[int] = []
    color_treatments: List[str] = []
    undeclared_intensity: List[str] = []
    ...
    for primitive in recipe.primitives:
        ...
        if op in {"ZOOM", "SCALE"} and "scale_delta_bps" not in primitive and "amount_bps" not in primitive:
            undeclared_intensity.append(f"{op} must declare amount_bps or scale_delta_bps")
        if op == "REFRAME" and "reframe_bps" not in primitive:
            undeclared_intensity.append("REFRAME must declare reframe_bps")
        if op == "MOTION" and "motion_amplitude_bps" not in primitive:
            undeclared_intensity.append("MOTION must declare motion_amplitude_bps")
        if "scale_delta_bps" in primitive:
            scale_deltas.append(abs(int(primitive["scale_delta_bps"])))
        if op in {"ZOOM", "SCALE"} and "amount_bps" in primitive:
            scale_deltas.append(abs(int(primitive["amount_bps"])))
        if "reframe_bps" in primitive:
            reframe_bps.append(abs(int(primitive["reframe_bps"])))
        if "motion_amplitude_bps" in primitive:
            motion_bps.append(abs(int(primitive["motion_amplitude_bps"])))
        if op in {"COLOR", "LUT", "COLOR_TREATMENT"}:
            color_treatments.append(str(primitive.get("treatment", "CONTROLLED")).upper())

    for keyframe in recipe.keyframes:
        if "scale_delta_bps" in keyframe:
            scale_deltas.append(abs(int(keyframe["scale_delta_bps"])))
        if "motion_amplitude_bps" in keyframe:
            motion_bps.append(abs(int(keyframe["motion_amplitude_bps"])))

    return {
        "scale_delta_bps": max(scale_deltas, default=0),
        "reframe_bps": max(reframe_bps, default=0),
        "motion_amplitude_bps": max(motion_bps, default=0),
        "color_treatments": color_treatments or ["NONE"],
        "undeclared_intensity": undeclared_intensity,
    }
...
def validate_transformation_for_source_quality(
    ...
    """Fail closed when a recipe exceeds deterministic source-quality limits."""

    limits = _TRANSFORMATION_QUALITY_LIMITS[profile.quality_level]
    declared = _declared_transform_intensity(recipe)
    violations: List[str] = list(declared["undeclared_intensity"])

    if declared["scale_delta_bps"] > limits["scale"]:
        violations.append(
            f"scale_delta_bps={declared['scale_delta_bps']} exceeds {limits['scale']} for {profile.quality_level}"
        )
    if declared["reframe_bps"] > limits["reframe"]:
        violations.append(
            f"reframe_bps={declared['reframe_bps']} exceeds {limits['reframe']} for {profile.quality_level}"
        )
    if declared["motion_amplitude_bps"] > limits["motion"]:
        violations.append(
            f"motion_amplitude_bps={declared['motion_amplitude_bps']} exceeds {limits['motion']} for {profile.quality_level}"
        )
    for treatment in declared["color_treatments"]:
        if treatment not in limits["color"]:
            violations.append(
                f"color treatment '{treatment}' is not allowed for {profile.quality_level}"
            )
...
def adapt_transformation_recipe_for_source_quality(
    ...
    """Return a deterministic, provenance-preserving bounded recipe projection."""
                    ...
                    for asset in element.asset_references:
                        ...
                        if asset.source_quality_profile is not None:
                            if asset.source_quality_profile.source_ref != asset.asset_id:
                                raise StoryboardRevisionValidationError(
                                    f"asset '{asset.asset_id}' has a source quality profile for another source"
                                )
                            if not set(asset.source_quality_profile.evidence_refs).issubset(evidence_refs):
                                raise StoryboardRevisionValidationError(
                                    f"asset '{asset.asset_id}' source quality profile references evidence outside revision lineage"
                                )
                    ...
                    if recipe is not None:
                        for asset in element.asset_references:
                            ...
                            if not quality_result.allowed:
                                raise StoryboardRevisionValidationError(
                                    f"element '{element.element_id}' exceeds source-quality limits: "
