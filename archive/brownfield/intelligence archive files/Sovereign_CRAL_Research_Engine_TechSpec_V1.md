...
- New dependency: DEP-RES-010 (Finding-Linked Source Cache)
- New dependency: DEP-RES-011 (Tribe Seed Phrases Registry)
- New dependency: DEP-RES-012 (Viral Signal Configuration)
...
@app.get("/api/viral_trends")
async def get_viral_trends():
    """
    Called by the Content Strategy Dashboard.
    Returns validated tribal trends across all coaches.
    """
    # Aggregate signals across all coaches, score by viral parameters
    pass
...
### 2.3 Dependency: Tribe Seed Phrases Registry (DEP-RES-011)

```json
{
  "dependency_id": "DEP-RES-011",
...
## Section 3 — Finding-Linked Source Cache (DEP-RES-010)
  ...
  "epistemic_friction_swarm": {
    ...
    "agents": [
      ...
      },
      {
        ...
        "tool_access": "SKILL-RES-SEARCH with category matching the stronger finding's moment",
        "mandatory_behavior": "MUST produce at least one counter-argument. If no genuine counter-evidence found, must explicitly state why the finding is robust."
      },
      {
        "name": "Contextualizer",
        "role": "Historical pattern matching",
        "action": "Cross-reference both findings against Finding-Linked Source Cache (DEP-RES-010). Has this trigger category produced similar conflicts before? How were they resolved?",
...
### 5.1 Dependency: Viral Signal Configuration (DEP-RES-012)

```json
{
  "dependency_id": "DEP-RES-012",
...
- Generate initial DEP-RES-011 seed phrases for all active coaches
...
|:---|:---|:---|
| DEP-RES-010 | Finding-Linked Source Cache | NEW — Redis DB 2, Tier 0 promotion at convergence 3 |
| DEP-RES-011 | Tribe Seed Phrases Registry | NEW — per-coach seed phrases for autocomplete polling |
| DEP-RES-012 | Viral Signal Configuration | NEW — 14 parameters computed from SearXNG JSON |
