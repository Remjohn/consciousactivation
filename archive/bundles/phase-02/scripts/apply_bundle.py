...
def rollback_from_receipt(repo,receipt,force=False):
    for record in reversed(receipt["files"]):
        ...
        if target.exists() and not force and sha256(target)!=record["new_sha256"]: raise RuntimeError(f"applied file drifted: {record['path']}")
        if record["operation"]=="create":
            if target.exists(): target.unlink()
        else:
            backup=Path(record["backup_path"])
            if not backup.is_file(): raise RuntimeError(f"missing backup: {backup}")
    ...
    for cache in sorted(repo.rglob("*"), key=lambda p: len(p.parts), reverse=True):
        if cache.is_dir() and cache.name in transient_names:
...
def main():
    ...
    plan={"schema_version":"ca-phase02-apply-plan/v1","bundle_id":bundle_id,"repo":str(repo),"actions":actions,"failures":failures,"result":"PASS" if not failures else "FAIL"}; print(json.dumps(plan,indent=2,sort_keys=True))
    ...
    try:
        ...
        receipt={"schema_version":"ca-phase02-apply-receipt/v1","bundle_id":bundle_id,"applied_at_utc":utc_now(),"repo":str(repo),"files":applied,"created_directories":[str(x.relative_to(repo)).replace("\\","/") for x in sorted(created_dirs,key=lambda q:(len(q.parts),str(q)))],"tests":"not_run" if a.skip_tests else "pending","result":"APPLIED_TESTS_SKIPPED" if a.skip_tests else "APPLIED_PENDING_TESTS"}
        ...
        if not a.skip_tests:
            report=state/"validation_report.json"; completed=subprocess.run([sys.executable,str(repo/"scripts"/"phase2"/"validate_phase2.py"),"--report",str(report)],cwd=repo,text=True)
        ...
        if a.commit:
            ...
            git(repo,"add","--all"); git(repo,"commit","-m","implement Phase 2 Activative Intelligence Runtime core")
