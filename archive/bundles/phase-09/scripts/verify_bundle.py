...
def verify(bundle_root: Path) -> dict[str, object]:
    ...
    for entry in manifest["files"]:
        ...
        if sha256(path)!=entry["sha256"]: failures.append({"path":entry["path"],"error":"sha256 mismatch"})
    ...
    return {"schema_version":"ca-phase09-bundle-verification/v1","bundle_id":manifest["bundle_id"],"file_count":len(manifest["files"]),"operation_count":len(operations["operations"]),"failures":failures,"result":"PASS" if not failures else "FAIL"}
