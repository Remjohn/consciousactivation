...
def verify(bundle_root: Path) -> dict[str, object]:
    ...
    return {"schema_version":"ca-phase08-bundle-verification/v1","bundle_id":manifest["bundle_id"],"file_count":len(manifest["files"]),"operation_count":len(operations["operations"]),"failures":failures,"result":"PASS" if not failures else "FAIL"}
