...
def main():
    ...
    plan={"schema_version":"ca-phase04-apply-plan/v1","bundle_id":bundle_id,"repo":str(repo),"actions":actions,"failures":failures,"result":"PASS" if not failures else "FAIL"}; print(json.dumps(plan,indent=2,sort_keys=True))
    ...
    try:
        ...
        receipt={"schema_version":"ca-phase04-apply-receipt/v1","bundle_id":bundle_id,"applied_at_utc":utc_now(),"repo":str(repo),"files":applied,"created_directories":[str(x.relative_to(repo)).replace("\\","/") for x in sorted(created_dirs,key=lambda q:(len(q.parts),str(q)))],"tests":"not_run" if a.skip_tests else "pending","result":"APPLIED_TESTS_SKIPPED" if a.skip_tests else "APPLIED_PENDING_TESTS"}
        ...
        if not a.skip_tests:
            report=state/"validation_report.json"; env=dict(os.environ); env["PYTHONDONTWRITEBYTECODE"]="1"; env["PYTHONWARNINGS"]="error::ResourceWarning"
            completed=subprocess.run([sys.executable,str(repo/"scripts"/"phase4"/"validate_phase4.py"),"--report",str(report)],cwd=repo,text=True,env=env)
        ...
        if a.commit:
            ...
            git(repo,"add","--all"); git(repo,"commit","-m","implement Phase 4 Interview Expression core")
