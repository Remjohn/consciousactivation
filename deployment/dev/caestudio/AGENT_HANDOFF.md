...
The supplied M0095 bundle was integrated without shared-file collisions. Focused M0095 tests pass 9/9; the selected affected regression passes 153/153 with the M0066 test-only module shim isolated in a separate pytest process, plus M0066 6/6 and Studio 20/20. The active Windows environment has no nginx executable, so native nginx syntax validation remains an explicit limitation rather than an unverified claim. See `docs/cae/specs/M0095/M0095_ASSEMBLY_GRAPH.md` and the updated evidence receipt.
