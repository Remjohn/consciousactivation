...
title: Selective Repair, Invalidation, Repair Limits, and Human Escalation
...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-EVAL-003.md
controlling_frs: ['FR-095', 'FR-096']
controlling_stories: ['ST-09.03']
upstream_draft_dependencies: ['TS-EVAL-002', 'TS-AIR-019', 'TS-AHP-005']
---

# TS-EVAL-003 - Selective Repair, Invalidation, Repair Limits, and Human Escalation

This is an implementation-grade candidate Tech Spec for the independent evaluation, failure attribution, selective repair and escalation boundary. It was written under the Prompt 02C specification-work authorization and ends at `WRITTEN_PENDING_AUDIT`. It does not authorize implementation, product adoption, build, release bytes, production use, certification, VAE Stage 5, or a Development Capsule.
...
|---|---|---|---|---|---|
| `SDE-082` | `TS-EVAL-002` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-EVAL-002.md` | `WRITTEN_PENDING_AUDIT` | `64c99efc49e60be93aea30e831b0082181f7151d6043e002f1ed5e0c4cac5144` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-083` | `TS-AIR-019` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-019.md` | `WRITTEN_PENDING_AUDIT` | `515e42a7e015c212f9f972b4e78e1e7aa0558816448f10ff18db9b9a7f7ecd5e` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-084` | `TS-AHP-005` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-005.md` | `WRITTEN_PENDING_AUDIT` | `e82cd9d256e5882c3891d70bad3e0fe660610202fd6ffcd5fb350950676d3e6f` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Selective Repair, Invalidation, Repair Limits, and Human Escalation` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.

The solution scope is the independent evaluation, failure attribution, selective repair and escalation boundary. The spec defines required inputs, state transitions, output artifacts, validation receipts, failure handling, migration behavior, rollback boundaries and test obligations. It excludes implementation source code, provider credentials, generated types, schemas, release packages, production signing and certification.
...
|---|---|---|---|
| `FR-095` | `Atomic Harness Pipeline` | `TS-EVAL-003` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-096` | `Atomic Harness Pipeline` | `TS-EVAL-003` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-09.03` | `TS-EVAL-003` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_EVAL_003_RuntimeContract`. It consumes TS-EVAL-002, TS-AIR-019, TS-AHP-005, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_EVAL_003_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_EVAL_003_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_EVAL_003_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_EVAL_003_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
