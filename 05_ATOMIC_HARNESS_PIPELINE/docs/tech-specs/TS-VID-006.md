...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-006.md
controlling_frs: ['FR-143', 'FR-144']
controlling_stories: ['ST-04.06']
upstream_draft_dependencies: ['TS-VID-005', 'TS-EVAL-001', 'TS-EVAL-002']
---
...
|---|---|---|---|---|---|
| `SDE-090` | `TS-VID-005` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-005.md` | `WRITTEN_PENDING_AUDIT` | `a1d4feb2d1a758493af972d132cc92fa4ed81964bafa60ed5d3a282e73418bc4` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-091` | `TS-EVAL-001` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-EVAL-001.md` | `WRITTEN_PENDING_AUDIT` | `0c3a47dc3cfb331630df794bec4869c71d5fc70894b6f0fd77b08068ff74024f` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-092` | `TS-EVAL-002` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-EVAL-002.md` | `WRITTEN_PENDING_AUDIT` | `64c99efc49e60be93aea30e831b0082181f7151d6043e002f1ed5e0c4cac5144` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Rendered Cut-Boundary Evaluation and Bounded Temporal Repair` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `FR-143` | `Atomic Harness Pipeline` | `TS-VID-006` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-144` | `Atomic Harness Pipeline` | `TS-VID-006` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-04.06` | `TS-VID-006` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_VID_006_RuntimeContract`. It consumes TS-VID-005, TS-EVAL-001, TS-EVAL-002, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_VID_006_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_VID_006_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_VID_006_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_VID_006_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
