...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-003.md
controlling_frs: ['FR-070', 'FR-142']
controlling_stories: ['ST-04.03']
upstream_draft_dependencies: ['TS-VID-002', 'TS-DEL-001']
---
...
|---|---|---|---|---|---|
| `SDE-074` | `TS-VID-002` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-002.md` | `WRITTEN_PENDING_AUDIT` | `ea8f83e732ac9da4ff8c31816ba9ad7e6c6a8bf866b1b6a5ae4c3bb4c9bc9b13` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-075` | `TS-DEL-001` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-DEL-001.md` | `WRITTEN_PENDING_AUDIT` | `aba43b66766795436b2073b528a486e7dbdb4cc48638ca21a1642c0e36e6d751` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Captions, Audio, Evidence, B-Roll, Reframing, and Motion-Slot Planning` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `FR-070` | `Atomic Harness Pipeline` | `TS-VID-003` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-142` | `Atomic Harness Pipeline` | `TS-VID-003` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-04.03` | `TS-VID-003` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_VID_003_RuntimeContract`. It consumes TS-VID-002, TS-DEL-001, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_VID_003_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_VID_003_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_VID_003_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_VID_003_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
