...
7. upstream specifications and contract seeds at their current hash-pinned quality states;
8. directly affected downstream specifications and their current quality states;
...
## Dependency-stage audit rule

Audit dependency edges according to their lifecycle class.

- A `WRITE_INTERFACE_DEPENDENCY` may have been consumed from a hash-pinned draft. Verify that the dependent spec labels it as non-accepted and remains compatible with the exact draft hash.
- An `ACCEPTANCE_PREREQUISITE` need not have been accepted when the dependent spec was written, but it must be accepted before the dependent spec receives final `ACCEPTED_FOR_BUILD`.
- A `BUILD_PREREQUISITE` must not be misused to block WRITE or AUDIT.
- A generic `depends_on` field without lifecycle classification is a blocking workflow defect.
- If an upstream audit finding changes an interface consumed downstream, record an explicit downstream revision impact.

## Repository write-authority audit rule

Verify the target path against the nearest `AGENTS.md`.

- Direct product specs must be in an allowed path.
- Program Control cross-product proposals must not modify the target product and must carry `PRODUCT_ADOPTION_REQUIRED`.
- A proposal may be audited for technical quality, but it cannot be recommended for product-local build readiness before adoption.
