...
from .media import FFmpegSourceLedRenderer, RemotionBindingCompiler, HyperFramesBindingCompiler, RenderedVideoEvaluator, SourceMediaService
...
def run_phase6_demo(database_path:str|Path|None=None,output_dir:str|Path|None=None)->dict[str,Any]:
    with tempfile.TemporaryDirectory(prefix='ca-phase6-demo-') as temp:
        ...
        source_reg=app.source_media.register(source_path=source,logical_uri='media/interview.mp4',source_package_ref=ref('ie:source-package','source'),transcript_alignment_ref=ref('ie:transcript','transcript'),visual_index_ref=ref('ie:visual-index','visual'),restrictions=['operator_source_authority'],idempotency_key='phase6-source')['object']['payload'];source_registration_ref=SourceMediaService.registration_ref(source_reg)
        ...
        edl=app.edls.compile(source_registration_ref=source_registration_ref,source_path=source,expression_moment_ref=ref('ie:moment','moment'),words=words,selections=[{'selection_id':'sel-1','start_word_id':'w1','end_word_id':'w2','function':'HOOK','cut_in_class':'WORD_BOUNDARY','cut_out_class':'WORD_BOUNDARY','authorized_reorder':False},{'selection_id':'sel-2','start_word_id':'w3','end_word_id':'w4','function':'CLAIM','cut_in_class':'WORD_BOUNDARY','cut_out_class':'AUDIO_EVENT_BOUNDARY','authorized_reorder':False}],allow_reorder=False,idempotency_key='phase6-edl')['object']['payload']
        duration=edl['output_duration_ms']
        request={'derivative_job_ref':ref('job:short','job'),'source_registration_ref':source_registration_ref,'semantic_production_package_ref':ref('air:semantic-package','semantic'),'final_script_ref':ref('air:final-script','script'),'activation_transfer_contract_ref':ref('air:transfer','transfer'),'harness_binding_ref':ref('binding:format07','binding'),'canvas':{'width':360,'height':640,'fps_numerator':30,'fps_denominator':1,'duration_ms':duration},'timebase':{'numerator':1,'denominator':1000},'tracks':[{'track_id':'track:a-roll','track_type':'VIDEO','role':'PRIMARY_A_ROLL_SPINE','z_index':0,'elements':[{'element_id':f"a-roll:{i}",'kind':'SOURCE_SEGMENT','output_start_ms':e['output_start_ms'],'output_end_ms':e['output_end_ms'],'semantic_role':'SOURCE_EXPRESSION','sequence_role':e['function'],'source_registration_ref':source_registration_ref,'source_start_ms':e['source_start_ms'],'source_end_ms':e['source_end_ms'],'artifact_ref':'NOT_APPLICABLE','generated_slot_state':'NOT_APPLICABLE','bbox_intent_ref':'NOT_APPLICABLE','text':'NOT_APPLICABLE'} for i,e in enumerate(edl['entries'])]}],'evaluation_profile_ref':ref('eval:video','eval'),'wrong_reading_locks':['do_not_replace_source_expression']}
        program=app.video_programs.compile(request,source_path=source,idempotency_key='phase6-program')['object']['payload']
        renderer=FFmpegSourceLedRenderer(app.repository);video=renderer.render(source_path=source,edl=edl,output_dir=out,logical_output_uri='video/source-led-short.mp4')
