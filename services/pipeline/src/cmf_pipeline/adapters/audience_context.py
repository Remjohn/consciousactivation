    ...
    def persist(
        ...
        *,
        ...
        for name, layer in (
            ...
            try:
                existing = repository.get_object(layer.layer_id, revision=layer.version)
                if existing["canonical_sha256"] == canonical_sha256(layer.to_dict()):
                    layer_results[name] = {"object": existing, "created": False}
                    continue
                else:
                    raise AudienceContextMutationError(
                        f"immutable layer {name} already exists at revision {layer.version} with different digest"
                    )
