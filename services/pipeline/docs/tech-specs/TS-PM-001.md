...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-PM-001.md
controlling_frs: ['FR-037', 'FR-038', 'FR-039', 'FR-040', 'FR-041', 'FR-042']
controlling_stories: ['ST-07.03']
upstream_draft_dependencies: ['TS-AHP-002', 'TS-AIR-020']
---

# TS-PM-001 - Programmed Model Artifact, Claim, Model Program, Lifecycle, and Resolver

This is an implementation-grade candidate Tech Spec for the Programmed Model resolver, claim, artifact and lifecycle boundary. It was written under the Prompt 02C specification-work authorization and ends at `WRITTEN_PENDING_AUDIT`. It does not authorize implementation, product adoption, build, release bytes, production use, certification, VAE Stage 5, or a Development Capsule.
...
|---|---|---|---|---|---|
| `SDE-114` | `TS-AHP-002` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-002.md` | `WRITTEN_PENDING_AUDIT` | `3e76ee7e4ec8f3b288a58bb8b8eb886195d9ad17097c67ded613c22fbb3dccd4` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-115` | `TS-AIR-020` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-020.md` | `WRITTEN_PENDING_AUDIT` | `b7c7a94f5a79b6db82cf2ba9ccbe7e59364882b50c849d4c30439bd53fff5584` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Programmed Model Artifact, Claim, Model Program, Lifecycle, and Resolver` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.

The solution scope is the Programmed Model resolver, claim, artifact and lifecycle boundary. The spec defines required inputs, state transitions, output artifacts, validation receipts, failure handling, migration behavior, rollback boundaries and test obligations. It excludes implementation source code, provider credentials, generated types, schemas, release packages, production signing and certification.
...
|---|---|---|---|
| `FR-037` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-038` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-039` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-040` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-041` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-042` | `Atomic Harness Pipeline` | `TS-PM-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-07.03` | `TS-PM-001` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_PM_001_RuntimeContract`. It consumes TS-AHP-002, TS-AIR-020, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_PM_001_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_PM_001_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_PM_001_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_PM_001_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
