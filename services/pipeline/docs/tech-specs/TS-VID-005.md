...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-005.md
controlling_frs: ['FR-073', 'FR-074', 'FR-075', 'FR-076', 'FR-077', 'FR-078']
controlling_stories: ['ST-04.05']
upstream_draft_dependencies: ['TS-VID-004']
---

# TS-VID-005 - Remotion, HyperFrames, FFmpeg Render, QA, and Export Adapters

This is an implementation-grade candidate Tech Spec for the short-form edited video temporal planning, render, evaluation and repair boundary. It was written under the Prompt 02C specification-work authorization and ends at `WRITTEN_PENDING_AUDIT`. It does not authorize implementation, product adoption, build, release bytes, production use, certification, VAE Stage 5, or a Development Capsule.
...
|---|---|---|---|---|---|
| `SDE-085` | `TS-VID-004` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-004.md` | `WRITTEN_PENDING_AUDIT` | `b04db0e51f33120419c62d7eb9521a79b337b9858b621d7b66917e30354fb473` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Remotion, HyperFrames, FFmpeg Render, QA, and Export Adapters` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.

The solution scope is the short-form edited video temporal planning, render, evaluation and repair boundary. The spec defines required inputs, state transitions, output artifacts, validation receipts, failure handling, migration behavior, rollback boundaries and test obligations. It excludes implementation source code, provider credentials, generated types, schemas, release packages, production signing and certification.
...
|---|---|---|---|
| `FR-073` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-074` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-075` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-076` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-077` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-078` | `Atomic Harness Pipeline` | `TS-VID-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-04.05` | `TS-VID-005` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_VID_005_RuntimeContract`. It consumes TS-VID-004, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_VID_005_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_VID_005_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_VID_005_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_VID_005_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
