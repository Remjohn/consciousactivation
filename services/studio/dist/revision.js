...
import { StudioValidationError, validateChangeProgram, validateDirectDelta, validateOperatorRequest, validateToolRegistry, } from "./validators.js";
export const DEFAULT_STUDIO_TOOLS = Object.freeze([
...
function targetLayer(request, context) {
    const first = request.target_refs[0];
    if (!first)
        throw new StudioValidationError("TARGET_REQUIRED", "revision request requires a target");
    return context.target_layers_by_ref[first.object_id] ?? "COMPOSITION";
}
function operationFromTemplate(template, request, targetLayerValue, index) {
...
function recipeMatch(request, context, normalized) {
    for (const recipe of context.steering_recipes) {
        if (!recipe.trigger_phrases.some((phrase) => normalized.includes(phrase.toLocaleLowerCase("en-US"))))
            continue;
...
function oneOperation(request, targetLayerValue, template) {
    return [operationFromTemplate(template, request, targetLayerValue, 0)];
}
function compileKnownLanguage(request, context) {
...
function finishProgram(args) {
    const body = {
        ...
        declared_invariants: uniqueSorted([...args.context.default_invariants,
...
export function compileNaturalLanguageRevision(request, context) {
    ...
    const requestRef = { object_id: request.request_id, version: "1.0.0", sha256: canonicalSha256(request) };
...
export function compileDirectManipulation(delta, context) {
    validateDirectDelta(delta);
    if (delta.expected_state_version !== context.state_version)
        throw new StudioValidationError("STALE_STATE_VERSION", "direct manipulation targets a stale state");
    ...
    const requestRef = { object_id: delta.delta_id, version: "1.0.0", sha256: canonicalSha256(delta) };
