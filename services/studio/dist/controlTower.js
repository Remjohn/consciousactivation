...
function availableActions(input) {
    ...
    if (input.timeline)
        actions.push("OPEN_TIMELINE", "REQUEST_REVISION", "DIRECT_MANIPULATION");
    if (input.artifacts.length)
        actions.push("COMPARE_ARTIFACTS", "REQUEST_REVISION");
    if (input.exception_packages.length)
        actions.push("RESOLVE_EXCEPTION");
    if (input.campaign.lifecycle_state === "READY_TO_SHIP")
        actions.push("REQUEST_SHIP_DECISION");
    return uniqueSorted(actions);
}
export function buildControlTowerProjection(input) {
...
export function controlTowerSummary(projection) {
    return {
