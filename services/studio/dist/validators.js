...
export function requireNonEmpty(value, label) {
    if (!value.trim())
        throw new StudioValidationError("EMPTY_VALUE", `${label} must not be empty`, { label });
}
...
export function validateImmutableRef(ref, label = "ref") {
    const legacy = ref;
...
export function validateActor(actor) {
    ...
    const legacy = actor;
    if (!actor.product_id && legacy.actor_type && legacy.display_name)
        return;
    requireNonEmpty(actor.product_id, "product_id");
}
export function validateCampaignOrder(order) {
    ...
    const legacySource = order.source_ref;
    const legacyShape = Boolean(legacySource.content_sha256);
    if (order.output_targets.length === 0 && !legacyShape)
        throw new StudioValidationError("OUTPUT_TARGET_REQUIRED", "at least one output target is required");
    for (const target of order.output_targets)
        requireSafeInteger(target.quantity, "output_target.quantity", 1);
...
export function validateToolRegistry(tools) {
    ...
    for (const tool of tools) {
        ...
        if (registry.has(tool.tool_id))
            throw new StudioValidationError("DUPLICATE_TOOL", `duplicate tool ${tool.tool_id}`);
        registry.set(tool.tool_id, tool);
    }
    return registry;
}
export function validateOperatorRequest(request) {
...
export function validateDirectDelta(delta) {
...
export function validateDependencyGraph(nodes) {
    ...
    if (ids.size !== nodes.length)
        throw new StudioValidationError("DUPLICATE_GRAPH_NODE", "dependency graph node IDs must be unique");
    for (const node of nodes) {
        for (const dependency of node.dependency_ids) {
            if (!ids.has(dependency))
                throw new StudioValidationError("UNKNOWN_GRAPH_DEPENDENCY", `${node.node_id} depends on unknown ${dependency}`);
        }
    }
}
...
export function validateChangeProgram(program, tools, allowedNodeIds) {
    if (program.compilation_status !== "COMPILED")
        throw new StudioValidationError("PROGRAM_NOT_COMPILED", "only compiled programs may execute");
    ...
    if (program.confidence_micros > 1_000_000)
        throw new StudioValidationError("CONFIDENCE_RANGE", "confidence_micros must be <= 1,000,000");
    if (program.exact_operations.length === 0)
        throw new StudioValidationError("OPERATION_REQUIRED", "compiled program requires an operation");
    if (program.exact_operations.length > 4)
        throw new StudioValidationError("PROGRAM_NOT_MINIMAL", "bounded Phase 7 programs may contain at most four operations");
    ...
    for (const operation of program.exact_operations) {
        ...
        if (!tool || tool.tool_version !== operation.tool_version)
            throw new StudioValidationError("TOOL_NOT_ALLOWED", `tool ${operation.tool_id}@${operation.tool_version} is not allowed`);
        if (!tool.allowed_target_layers.includes(operation.target_layer))
            throw new StudioValidationError("TOOL_LAYER_DENIED", `tool ${tool.tool_id} cannot target ${operation.target_layer}`);
        if (!allowed.has(operation.target_node_id))
            throw new StudioValidationError("NODE_SCOPE_DENIED", `target node ${operation.target_node_id} is outside scope`);
        const keys = Object.keys(operation.arguments).sort();
        for (const key of keys)
            if (!tool.argument_keys.includes(key))
                throw new StudioValidationError("ARGUMENT_DENIED", `argument ${key} is not accepted by ${tool.tool_id}`);
        const signature = `${operation.tool_id}|${operation.target_ref.object_id}|${JSON.stringify(operation.arguments)}`;
        if (signatures.has(signature))
            throw new StudioValidationError("DUPLICATE_OPERATION", "duplicate operations violate minimality");
