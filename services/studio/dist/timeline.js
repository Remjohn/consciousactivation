...
function msToFrame(ms, numerator, denominator) {
    return Math.round((ms * numerator) / (1000 * denominator));
}
export function projectVideoEditProgram(program) {
    ...
    for (const track of [...program.tracks].sort((a, b) => a.z_index - b.z_index || a.track_id.localeCompare(b.track_id))) {
        ...
        for (const element of [...track.elements].sort((a, b) => a.output_start_ms - b.output_start_ms || a.element_id.localeCompare(b.element_id))) {
            if (itemIds.has(element.element_id))
                throw new StudioValidationError("DUPLICATE_TIMELINE_ITEM", `duplicate timeline item ${element.element_id}`);
            if (element.output_end_ms <= element.output_start_ms)
                throw new StudioValidationError("INVALID_TIMELINE_RANGE", `invalid range for ${element.element_id}`);
    ...
    const ref = { object_id: program.program_id, version: "1.0.0", sha256: program.program_sha256 };
