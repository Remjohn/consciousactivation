...
export function defaultAutonomyPolicy(mode) {
    return {
        mode,
...
export function createCampaignOrder(input) {
    const order = { ...input, order_id: deterministicId("campaign-order", input) };
    validateCampaignOrder(order);
    return order;
}
export function launchCampaign(order) {
    validateCampaignOrder(order);
...
export function transitionCampaign(state, next, updates = {}) {
...
export function shouldInterruptOperator(state, checkpointId, hasException) {
    if (hasException)
        return true;
    if (state.autonomy_mode === "SHADOW")
        return false;
    if (state.autonomy_mode === "CHECKPOINTED")
        return checkpointId !== null;
    if (state.autonomy_mode === "REVIEW_BEFORE_SHIP")
        return checkpointId === "final-artifact-review";
    return false;
}
