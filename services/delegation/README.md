# CMF Content Harness ↔ Visual Asset Editor Delegation Protocol — V1.1 Aligned Reference

**Contract package:** `1.1.0-rc.2`  
**Envelope / demand versions:** `1.0` / `1.1`  
**Artifact status:** `local_release_candidate`  
...
The V1.1 alignment preserves exact Activative semantic lineage, Activation
Contract, Visual Semantic/Narrative programs, Feature Contracts, T/V routing,
Reaction Receipt and Expression Moment lineage, and non-empty wrong-reading
locks. Compatibility is behavioral; parsing alone does not qualify.
The RC2 correction adds governed `source_provenance.source_kind`, enforces
interview-specific Reaction Receipt and Expression Moment provenance, and
validates the complete release from a clean release-only directory. RC1 is
retained as consumer-rejected historical evidence.
...
- [Stage 5 implementation report](docs/implementation/STAGE5_IMPLEMENTATION_REPORT.md)
- [Implementation readiness](docs/implementation/IMPLEMENTATION_READINESS.md)
...
The active generated implementation family under `packages/` contains 26
closed schemas/examples, generated bindings, deterministic validators,
compatibility/migration fixtures, and the transport-neutral reference engine.
...
1. Obtain Content Harness and VAE owner ratification and pinned product releases.
2. Execute the canonical vectors through real product adapters.
3. Supply trust roots, signing/key lifecycle, transport ports, durable state,
   audit/outbox, recovery, and retention ownership.
4. Verify the existing Control Tower integration contract.
5. Publish a signed immutable release only after those external gates pass.

No current artifact authorizes production use.
...
python scripts/validate_release_clean_room.py delegation-contracts/1.1.0-rc.2
```
