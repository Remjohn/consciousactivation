...
class AuthorityTests(unittest.TestCase):
    ...
    def test_constitutional_demand_paths_have_content_harness_authority(self) -> None:
        protected_paths = [
            "/activative_semantic_lineage/activative_intelligence_pack_ref/version",
            "/activative_semantic_lineage/reaction_receipt_refs/0/payload_hash",
            "/activative_semantic_lineage/expression_moment_refs/0/canonical_ref",
            "/activation_contract/viewer_roles/0",
            "/visual_semantic_pack/recognition_intent",
            "/visual_narrative_program/attention_state_sequence/0",
            "/feature_contracts/0/contract_ref/resource_id",
            "/somatic_route_request/t_codes/0",
            "/wrong_reading_locks/0",
        ]
        for path in protected_paths:
            self.assertEqual(resolve_value_owner("visual-asset-demand", path), "CONTENT_HARNESS")
        with self.assertRaises(AuthorityError):
            validate_authority("visual-asset-demand", "VISUAL_ASSET_EDITOR")
