...
def build_parser() -> argparse.ArgumentParser:
    ...
    command_parser("demo")
    production_demo = command_parser("production-demo")
    production_demo.add_argument("--interview-database", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "status":
        ...
        command, command_payload, event, event_payload, receipt = bootstrap_transition(
            "activative-intelligence-runtime", "0.3.0-dev.1"
        )
    ...
    elif args.command == "production-demo":
        from .production_demo import run_production_demo

        interview_database = args.interview_database or app.repository.path.with_name("interview-expression-demo.sqlite")
        result = run_production_demo(app.repository.path, interview_database)
