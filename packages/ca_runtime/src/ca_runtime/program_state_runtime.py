            ...
            try:
                ...
                if expected_version is not None:
                    if aggregate.version != expected_version + 1:
                        raise ProgramStateRuntimeError(
                            f"CAS transition version contract violated for aggregate '{aggregate.aggregate_id}': "
                            f"expected_version={expected_version}, proposed_version={aggregate.version}",
                            reason_code="CAS_VERSION_PROPOSAL_MISMATCH",
                            details={
                                "aggregate_id": aggregate.aggregate_id,
                                "expected_version": expected_version,
                                "proposed_version": aggregate.version,
                            },
                        )
                    try:
                        cas_update_program_state_aggregate(
                    ...
                    except SqliteCasVersionMismatchError as error:
                        raise ProgramStateVersionConflictError(
                            aggregate_id=error.aggregate_id,
                            expected_version=error.expected_version,
                            actual_version=error.actual_version,
                        ) from error
                    except SqliteCasAggregateNotFoundError as error:
                        raise ProgramStateAggregateNotFoundError(error.aggregate_id) from error
                else:
                    row = conn.execute(
                        "SELECT aggregate_id FROM cae_program_state_aggregates WHERE aggregate_id = ?",
                        (aggregate.aggregate_id,),
