...
def _launch_argv(executable: str, args: Sequence[str]) -> list[str]:
    """Build a shell=False argv, mapping POSIX ``echo`` onto Windows when needed."""

    basename = Path(executable).name
    if (
        os.name == "nt"
        and basename.lower() == "echo"
        and shutil.which(executable) is None
    ):
        printer = "import sys; sys.stdout.write(' '.join(sys.argv[1:]) + '\\n')"
        return [sys.executable, "-c", printer, *list(args)]
...
class ToolSandbox:
    ...
    def execute_tool(
        ...
        *,
        ...
        argv = _launch_argv(executable, args)
        logger.info(
