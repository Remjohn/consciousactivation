        ...
        *,
        ...
        for position, binding in enumerate(ordered):
            ...
            if not has_required_prior:
                errors.append(
                    f"scene '{binding.scene_id}' {entry.mode} requires a prior mode "
                    f"(requires prior mode in {list(entry.requires_prior_modes)})"
                    f"{list(entry.requires_prior_modes)}"
                )
            if not has_required_later:
                errors.append(
                    f"scene '{binding.scene_id}' {entry.mode} requires a later mode "
                    f"(requires later mode in {list(entry.requires_later_modes)})"
        ...
        if any("requires prior mode" in error for error in errors) or any(
            "requires later mode" in error for error in errors
        ):
