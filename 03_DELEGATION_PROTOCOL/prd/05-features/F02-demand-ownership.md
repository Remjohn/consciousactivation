...
## RC2 consumer-conformance correction

Every Visual Asset Demand carries the mandatory governed discriminator
`source_provenance.source_kind`. Free-form or unknown source kinds are invalid.
When the value is `interview_expression`, non-empty Reaction Receipt and
Expression Moment reference collections are mandatory. Other governed source
kinds may omit those collections, but supplied references remain fully typed
and validated. The protocol and every adapter preserve these values unchanged;
they do not infer source kind or manufacture missing interview provenance.
