...
## Constitutional V1.1 handoff amendment

Architecture must consume package candidate `1.1.0-rc.1` while keeping the
envelope at protocol `1.0` and Visual Asset Demand at message `1.1`. It must
preserve exact upstream identity/version/hash/reference tuples for Activative
Intelligence, Identity DNA, Context Premise, Resonance, Matrix of Edging,
Activative Call, Reaction Receipt, and Expression Moment lineage. Compatibility
admission must require behavioral preservation/enforcement and evaluator
evidence; no adapter may silently default or remove mandatory meaning.
