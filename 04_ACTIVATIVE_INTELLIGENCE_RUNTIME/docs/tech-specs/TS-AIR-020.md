...
primary_owner: Activative Intelligence Runtime
...
output_path: 04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-020.md
controlling_frs: ['AIR-FR-115', 'AIR-FR-116', 'AIR-FR-117', 'AIR-FR-118', 'AIR-FR-119', 'AIR-FR-120', 'FR-099', 'FR-102']
controlling_stories: ['AIR-ST-20.01', 'AIR-ST-20.02', 'AIR-ST-20.03', 'ST-10.02']
upstream_draft_dependencies: ['TS-AIR-003', 'TS-AIR-005', 'TS-AIR-018', 'TS-AIR-019']
---

# TS-AIR-020 - Programmed Models, Independent Evaluation, Product Integration, and Evidence

This is an implementation-grade candidate Tech Spec for the AIR semantic lifecycle, scoped learning, programmed-model meaning and evidence boundary. It was written under the Prompt 02C specification-work authorization and ends at `WRITTEN_PENDING_AUDIT`. It does not authorize implementation, product adoption, build, release bytes, production use, certification, VAE Stage 5, or a Development Capsule.
...
|---|---|---|---|---|---|
| `SDE-108` | `TS-AIR-003` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-003.md` | `WRITTEN_PENDING_AUDIT` | `ce9ac739346789d115ada80c44b568c28e61ce68f0ae99bb55b0962c875d430c` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-109` | `TS-AIR-005` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-005.md` | `WRITTEN_PENDING_AUDIT` | `5dcf631efc4f68f40edd0991ca3ae4be9073da68aa441c52d37e874e70a43e49` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-110` | `TS-AIR-018` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-018.md` | `WRITTEN_PENDING_AUDIT` | `cd817a664a574efe526daccb81d9ee299f3fdeaf9b999710278173423bceb753` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-111` | `TS-AIR-019` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-019.md` | `WRITTEN_PENDING_AUDIT` | `515e42a7e015c212f9f972b4e78e1e7aa0558816448f10ff18db9b9a7f7ecd5e` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Programmed Models, Independent Evaluation, Product Integration, and Evidence` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.

The solution scope is the AIR semantic lifecycle, scoped learning, programmed-model meaning and evidence boundary. The spec defines required inputs, state transitions, output artifacts, validation receipts, failure handling, migration behavior, rollback boundaries and test obligations. It excludes implementation source code, provider credentials, generated types, schemas, release packages, production signing and certification.

## 3. Governing decisions and non-authority boundaries

- `Activative Intelligence Runtime` owns only the behavior explicitly assigned by the Prompt 02 ledger and this packet.
...
|---|---|---|---|
| `AIR-FR-115` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-116` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-117` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-118` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-119` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-120` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-099` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-102` | `Activative Intelligence Runtime` | `TS-AIR-020` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `AIR-ST-20.01` | `TS-AIR-020` | `WRITTEN_PENDING_AUDIT` |
| `AIR-ST-20.02` | `TS-AIR-020` | `WRITTEN_PENDING_AUDIT` |
| `AIR-ST-20.03` | `TS-AIR-020` | `WRITTEN_PENDING_AUDIT` |
| `ST-10.02` | `TS-AIR-020` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_AIR_020_RuntimeContract`. It consumes TS-AIR-003, TS-AIR-005, TS-AIR-018, TS-AIR-019, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_AIR_020_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_AIR_020_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_AIR_020_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_AIR_020_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
