...
output_path: 04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-018.md
controlling_frs: ['AIR-FR-107', 'AIR-FR-108', 'FR-101']
controlling_stories: ['AIR-ST-18.03', 'ST-10.02']
upstream_draft_dependencies: ['TS-CAS-003', 'TS-AIR-011']
---
...
|---|---|---|---|---|---|
| `SDE-102` | `TS-CAS-003` | `07_CONSCIOUS_ACTIVATIONS_STUDIO/docs/tech-specs/TS-CAS-003.md` | `WRITTEN_PENDING_AUDIT` | `2d8642d1b8e7f170732fbd293bc2d86997b3568756c5e686857e9c790f3f5841` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-103` | `TS-AIR-011` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-011.md` | `WRITTEN_PENDING_AUDIT` | `c48ef679872bfa3e8bf2bd40ea44c3f4d18da30b3d05d19a99d5131fe26dd00b` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Human Resolution Episodes and Scoped Learning` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `AIR-FR-107` | `Activative Intelligence Runtime` | `TS-AIR-018` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `AIR-FR-108` | `Activative Intelligence Runtime` | `TS-AIR-018` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-101` | `Activative Intelligence Runtime` | `TS-AIR-018` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `AIR-ST-18.03` | `TS-AIR-018` | `WRITTEN_PENDING_AUDIT` |
| `ST-10.02` | `TS-AIR-018` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_AIR_018_RuntimeContract`. It consumes TS-CAS-003, TS-AIR-011, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_AIR_018_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_AIR_018_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_AIR_018_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_AIR_018_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
