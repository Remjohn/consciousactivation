...
def rollback_from_receipt(repo,receipt,force=False):
    for record in reversed(receipt["files"]):
        ...
        if record["operation"]=="create":
            ...
            if observed!=record["new_sha256"] and not force:
                raise RuntimeError(f"applied file drifted: {record['path']}")
            target.unlink()
        else:
            old_sha=record.get("old_sha256")
            if target.exists():
                observed=sha256(target)
                if old_sha and observed==old_sha:
                    continue
                if observed!=record["new_sha256"] and not force:
                    raise RuntimeError(f"applied file drifted: {record['path']}")
            backup=Path(record["backup_path"])
            if not backup.is_file():
                if target.exists() and old_sha and sha256(target)==old_sha:
                    continue
                raise RuntimeError(f"missing backup: {backup}")
            target.parent.mkdir(parents=True,exist_ok=True); backup.replace(target)
    ...
    for cache in sorted(repo.rglob("*"), key=lambda p: len(p.parts), reverse=True):
        if cache.is_dir() and (cache.name in transient_names or cache.name.endswith(".egg-info")):
            shutil.rmtree(cache, ignore_errors=True)
        elif cache.is_file() and cache.suffix in {".pyc", ".pyo"}:
...
def main():
    ...
    plan={"schema_version":"ca-phase03-apply-plan/v1","bundle_id":bundle_id,"repo":str(repo),"actions":actions,"failures":failures,"result":"PASS" if not failures else "FAIL"}; print(json.dumps(plan,indent=2,sort_keys=True))
    ...
    try:
        ...
        receipt={"schema_version":"ca-phase03-apply-receipt/v1","bundle_id":bundle_id,"applied_at_utc":utc_now(),"repo":str(repo),"files":applied,"created_directories":[str(x.relative_to(repo)).replace("\\","/") for x in sorted(created_dirs,key=lambda q:(len(q.parts),str(q)))],"tests":"not_run" if a.skip_tests else "pending","result":"APPLIED_TESTS_SKIPPED" if a.skip_tests else "APPLIED_PENDING_TESTS"}
        ...
        if not a.skip_tests:
            report=state/"validation_report.json"; completed=subprocess.run([sys.executable,str(repo/"scripts"/"phase3"/"validate_phase3.py"),"--report",str(report)],cwd=repo,text=True)
        ...
        if a.commit:
            ...
            git(repo,"add","--all"); git(repo,"commit","-m","implement Phase 3 Atomic Harness Pipeline core")
