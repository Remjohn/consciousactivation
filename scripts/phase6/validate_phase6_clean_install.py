...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='ca-phase6-install-') as td:
  t=Path(td);install=t/'install';build=t/'build';sources=[(ROOT/'packages/ca_contracts',build/'contracts'),(ROOT/'packages/ca_runtime',build/'runtime'),(ROOT/'packages/ca_delegation_rc4',build/'delegation'),(ROOT/'services/air',build/'air'),(ROOT/'services/pipeline',build/'pipeline'),(ROOT/'services/interview',build/'interview')]
