...
TRACE = ROOT / "governance/program-control" / "03_PROGRAM_STATUS" / "PHASE_09_FINAL_APPLICATION"
PYTHON_SOURCES = [
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ca-phase9-") as td:
        ...
        checks=[]
        checks.append(run([sys.executable,"-m","compileall","-q","packages/ca_release/src","services/air/src","services/pipeline/src","services/interview/src","services/vae/src","packages/ca_contracts/src","packages/ca_runtime/src","packages/ca_delegation_rc4/src"],env))
