"""Apply the guarded WP-03A immutable-evidence payload migration."""
...
MIGRATION_VERSION = "0003_cae_immutable_evidence_payloads"
MIGRATION_PATH = Path("docs/cae/implementation/sql/0003_cae_immutable_evidence_payloads.sql")
MIGRATION_ACTOR = "cae-wp03a-evidence-runner"
    ...
    try:
        with psycopg.connect(connection_url(), connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT count(*) FROM cae.command UNION ALL SELECT count(*) FROM cae.event UNION ALL SELECT count(*) FROM cae.receipt"
                )
                counts = tuple(row[0] for row in cursor.fetchall())
            if arguments.check:
                ...
                print(f"evidence_rows_empty={str(all(count == 0 for count in counts)).lower()}")
                return 0
            ...
            if any(count != 0 for count in counts):
                print("migration_status=REFUSED_EVIDENCE_ROWS_EXIST")
                return 2
