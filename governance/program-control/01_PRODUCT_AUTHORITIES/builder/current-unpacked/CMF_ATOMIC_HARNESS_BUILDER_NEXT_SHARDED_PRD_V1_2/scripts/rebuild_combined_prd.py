...
def rebase_links(text: str, source_rel: str) -> str:
    ...
    def repl(match):
        ...
        if target.startswith(("http://", "https://", "#", "mailto:", "asset://", "contract://", "visual-memory://", "benchmark://", "registry://")):
...
for rel in SECTION_ORDER:
    parts.append(rebase_links(strip_frontmatter((ROOT / rel).read_text(encoding="utf-8")), rel.replace("prd/", "", 1)))
