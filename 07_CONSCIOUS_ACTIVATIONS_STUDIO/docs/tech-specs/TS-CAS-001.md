...
output_path: 07_CONSCIOUS_ACTIVATIONS_STUDIO/docs/tech-specs/TS-CAS-001.md
controlling_frs: ['FR-151', 'FR-153']
controlling_stories: ['ST-10.03']
upstream_draft_dependencies: ['TS-EVAL-003', 'TS-AHP-005']
---
...
|---|---|---|---|---|---|
| `SDE-088` | `TS-EVAL-003` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-EVAL-003.md` | `WRITTEN_PENDING_AUDIT` | `2632f1fcceb757a54b8a203b1cacc0c862c9fef1db964bbefcef2e6b67a818c6` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-089` | `TS-AHP-005` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-005.md` | `WRITTEN_PENDING_AUDIT` | `e82cd9d256e5882c3891d70bad3e0fe660610202fd6ffcd5fb350950676d3e6f` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Natural-Language Revision Compiler and ChangeRequestProgram` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `FR-151` | `Conscious Activations Studio` | `TS-CAS-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-153` | `Conscious Activations Studio` | `TS-CAS-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-10.03` | `TS-CAS-001` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_CAS_001_RuntimeContract`. It consumes TS-EVAL-003, TS-AHP-005, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_CAS_001_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_CAS_001_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_CAS_001_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_CAS_001_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
