...
output_path: 07_CONSCIOUS_ACTIVATIONS_STUDIO/docs/tech-specs/TS-CAS-005.md
controlling_frs: ['FR-111', 'FR-157', 'FR-158', 'FR-159', 'FR-160']
controlling_stories: ['ST-10.07']
upstream_draft_dependencies: ['TS-CAS-004']
---
...
|---|---|---|---|---|---|
| `SDE-107` | `TS-CAS-004` | `07_CONSCIOUS_ACTIVATIONS_STUDIO/docs/tech-specs/TS-CAS-004.md` | `WRITTEN_PENDING_AUDIT` | `72cece250ffb7e6de8d14012145bba8fc2ed18b917e0ddd99c527159ab215435` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Campaign Order, Shared Shell, Autonomy Modes, and Exception Review` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `FR-111` | `Conscious Activations Studio` | `TS-CAS-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-157` | `Conscious Activations Studio` | `TS-CAS-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-158` | `Conscious Activations Studio` | `TS-CAS-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-159` | `Conscious Activations Studio` | `TS-CAS-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-160` | `Conscious Activations Studio` | `TS-CAS-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-10.07` | `TS-CAS-005` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_CAS_005_RuntimeContract`. It consumes TS-CAS-004, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_CAS_005_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_CAS_005_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_CAS_005_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_CAS_005_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
