...
def _write_pytest_fixture(directory: Path, *, positive_passes: bool = True) -> Path:
    workspace_dir = REPO_ROOT / ".pytest_tmp" / f"m033_{id(directory)}"
    workspace_dir.mkdir(parents=True, exist_ok=True)
    test_path = workspace_dir / "test_boundary.py"
    if positive_passes:
        positive_body = (
        ...
        positive_body = "    assert False, 'intentional positive acceptance failure'\n"
