...
def test_cli_production_demo(tmp_path):
    ...
    completed=subprocess.run([sys.executable,'-m','cmf_activative_intelligence','--database',str(tmp_path/'air.sqlite3'),'production-demo','--interview-database',str(tmp_path/'ie.sqlite3'),'--json'],cwd=ROOT,env=env,text=True,capture_output=True,timeout=300)
