...
def test_cli_production_demo(tmp_path):
    env=dict(os.environ);env['PYTHONPATH']=os.pathsep.join(str(x) for x in [ROOT/'packages/ca_contracts/src',ROOT/'packages/ca_runtime/src',ROOT/'services/air/src',ROOT/'services/interview/src'])
