"""Apply the guarded WP-04 immutable registry-authority schema migration."""
...
MIGRATION_VERSION = "0005_cae_registry_authority"
MIGRATION_PATH = Path("docs/cae/implementation/sql/0005_cae_registry_authority.sql")
MIGRATION_ACTOR = "cae-wp04-registry-schema-runner"
    ...
    try:
        with psycopg.connect(connection_url(), connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT to_regclass('cae.registry_snapshot'), to_regclass('cae.registry_item')"
                )
            ...
            if arguments.check:
                ...
                print(f"registry_tables_present={str(tables == ('cae.registry_snapshot', 'cae.registry_item')).lower()}")
            ...
            if any(tables):
                print("migration_status=REFUSED_REGISTRY_TABLES_EXIST")
                return 2
