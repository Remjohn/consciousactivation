...
PATHS=[ROOT/'tests/phase4',ROOT/'tests/phase3',ROOT/'tests/phase2',ROOT/'tests/phase1',ROOT/'packages/ca_contracts/src',ROOT/'packages/ca_runtime/src',ROOT/'services/air/src',ROOT/'services/pipeline/src',ROOT/'services/interview/src']
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='ca-phase4-') as td:
  ...
  checks=[]
  checks.append(run([sys.executable,'-m','compileall','-q','services/interview/src'],env))
  checks.append(run([sys.executable,'-m','pytest','tests/phase1','tests/phase2','tests/phase3','tests/phase4','services/interview/tests','-q'],env))
  ...
  for source,target in [(ROOT/'packages/ca_contracts',build/'ca_contracts'),(ROOT/'packages/ca_runtime',build/'ca_runtime'),(ROOT/'services/interview',build/'ie')]: shutil.copytree(source,target,ignore=shutil.ignore_patterns('build','dist','*.egg-info','__pycache__','*.pyc'))
