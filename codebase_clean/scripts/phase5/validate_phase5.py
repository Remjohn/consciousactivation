...
PATHS=[ROOT/'packages/ca_contracts/src',ROOT/'packages/ca_runtime/src',ROOT/'services/air/src',ROOT/'services/pipeline/src',ROOT/'services/interview/src']
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='ca-phase5-') as td:
        ...
        checks=[]
        checks.append(run([sys.executable,'-m','compileall','-q','services/air/src','services/interview/src'],env))
        suites=['tests/phase1','tests/phase2','tests/phase3','tests/phase4','tests/phase5','services/interview/tests']
        for suite in suites:
            suite_env=dict(env)
        ...
        for source,target in [(ROOT/'packages/ca_contracts',build/'ca_contracts'),(ROOT/'packages/ca_runtime',build/'ca_runtime'),(ROOT/'services/interview',build/'ie'),(ROOT/'services/air',build/'air')]:
