# M04 — Program State Coverage + StateM Runtime Mapping

Status: PROPOSED — OPERATOR-RATIFICATION-REQUIRED
Phase: 1 — Inventory and Contracts

## 1. Decision / Objective
Map every Program family to existing CAE State Aggregate/Transition/Transition Contract and determine executable coverage. Use StateM only as a runtime pattern: context boundary, in/out hooks, before-transfer checks, repair and recovery.

## 2. Mandatory reading
Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
M4-specific: docs/cae/state; relevant state migrations; workflow state; StateM paper.
...
## 3. Allowed work
Produce Program State Coverage Matrix and minimum reusable runtime state adapter requirements.
...
Every Program has explicit state coverage or a proven gap; no second state ontology.
...
## 9. Gemini execution
Execute ONLY M04. Do not begin another mandate. After the evidence report and operator decision request, STOP.
