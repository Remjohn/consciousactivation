# Mandate 54 — Typed Agent Result + Explicit Gate Engine
    ...
    Bring the model-output boundary up to the contract rigor already present in CAE handoffs, runtime nodes and state transitions.
...
- `services/pipeline/src/cmf_pipeline/workflow/application/handoff_validator.py`
- `services/pipeline/src/cmf_pipeline/workflow/domain/models.py`
    ...
    ### Allowed work
    Introduce a strict typed-result layer using declared output contracts. Preserve `ReasoningInferenceResult` as provider evidence but require a typed AgentResult before phase success. Implement explicit gate objects/functions for schema, required fields, evidence, authority and semantic validators.

    ### Required artifacts
    Typed result models; gate registry; validation receipts; parsing/error taxonomy; adversarial tests; CURRENT update.
    ...
    ```python
    typed = result_validator.parse(raw_response, output_contract)
checks = gate_engine.evaluate(typed)
assert checks.all_required_passed
    ```

    ### SSSF reference example
    ```text
    SSSF declares an `output_type` on the phase call and evaluates gate functions before continuing. CAE should adopt the same explicitness with stronger object/authority constraints.
    ```
...
- do not promote Agent-generated lessons into binding procedure without versioned validation through existing authority.
    ...
    - Invalid schema cannot succeed.
- Missing required artifact/evidence refs cannot succeed.
- Gate results are individually visible.
- Agent “done” text never equals completion.
- Existing HandoffValidator remains the downstream contract authority.
    ...
    Provider-facing parser changes must be reversible. Keep raw response evidence for debugging; do not discard it. Gate implementation must not mutate canonical objects directly.
    ...
    Pass arbitrary valid JSON with the wrong schema; return a success envelope with fake artifact refs; return a plausible narrative without required fields; deliberately omit one non-compensable gate.
