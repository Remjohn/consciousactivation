# Mandate 49 — Canonical Agent Constitution + Registry
    ...
    Make Agent a first-class, independently addressable CAE object. Formalize the Agent definition separately from Program assignment so one Agent can participate in Programs, reviews, debugging sessions, debates, or other governed sessions without duplicating its identity.
...
- `programs/research_canonicalization_program/program_manifest.yaml`
    ...
    ### Allowed work
    Define the canonical Agent object schema and registry using existing `AgentMemberSpec`, `SubagentSpec`, `CompiledAgentPackage`, and related authorities as evidence. Add only missing registry/constitution structures. Establish deterministic identity, version, authority lane, model policy, prompt references, CAE.md root, Skill/Tool/Capability/Hook/Harness references, output contracts, sub-agent policy, lifecycle and observability fields.

    ### Required artifacts
    Agent object constitution; registry schema; resolver; object authority mapping; tests; implementation receipt; `docs/PRD/CURRENT.md` update.
    ...
    ```python
    agent = app.agent_registry.resolve(agent_id="ResearchCommanderAgent", version="1.0.0")
assert agent.lifecycle_state == "APPROVED"
assert agent.authority_lane == "COMMANDER"
    ```

    ### SSSF reference example
    ```text
    SSSF keeps an explicit roster in `sssf.config.yaml`; the roster answers “who is this agent?” while an ADW decides how the agent is used. Adapt that separation to CAE.
    ```
    ...
    - An Agent is discoverable without a Program.
- The same Agent ID/version resolves identically in repeated runs.
- Program manifests can bind the registry Agent by reference.
- A standalone-session call can bind the same Agent.
- Invalid lifecycle/authority combinations fail closed.
    ...
    Registry/implementation changes are reversible. No existing Program package may be rewritten destructively. Preserve historical Agent references; if an identity collision is discovered, quarantine and stop.
    ...
    Attempt to register two different Agent bodies under one Agent ID/version; attempt to create an Agent with a lane or capability policy that violates its constitution; attempt to invoke an Agent that exists only as an inline Program name.
