# Mandate 52 — Canonical Agent Invocation Contract
    ...
    Create the single typed execution object that binds Agent identity, Program or Session context, state, compiled package, JIT capsule, model resolution, Skills, tools, capabilities, output contract and gates to one actual model invocation.
...
- `services/pipeline/src/cmf_pipeline/reasoning/model_reasoning_engine.py`
- `services/pipeline/src/cmf_pipeline/programmed_model_engine.py`
- `services/pipeline/src/cmf_pipeline/workflow/application/jit_context.py`
    ...
    ### Allowed work
    Define `AgentInvocation` using existing context-capsule and model-resolution fields. Introduce one adapter boundary to the provider/Pi runtime. Preserve `ReasoningInferenceResult`; make the invocation receipt include package/context/model/prompt identity. No caller should be able to bypass the invocation object for governed Agent work.

    ### Required artifacts
    Invocation object constitution; compiler/adapter; model bridge; receipt schema; integration tests; one real inference proof; CURRENT update.
    ...
    ```python
    invocation = invocation_compiler.compile(node, agent, jit_capsule, model_resolution)
...
assert result.receipt_id
    ...
    ```text
    SSSF’s phase call binds an agent phase to a prompt, an output type and gates. CAE should make that binding an explicit typed object with much stronger governance/provenance.
    ```
...
- do not promote Agent-generated lessons into binding procedure without versioned validation through existing authority.
    ...
    - A Program-owned Agent call produces an AgentInvocation.
- A standalone session produces the same invocation shape.
- Prompt/context/package/model hashes are captured.
- Tool/action constraints travel with the invocation.
- Bypass attempts are blocked or flagged.
    ...
    Keep provider adapters reversible. Do not migrate providers or models. If Pi/provider execution is unavailable, stop at a proven invocation-compilation boundary rather than fabricating provider evidence.
    ...
    Call `ModelReasoningEngine.infer()` directly without the compiled invocation; alter the context capsule after compilation; supply a tool not present in the invocation; use a model program that is not authorized for the Agent.
