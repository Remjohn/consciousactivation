# CAE-M08 — Scoring Clustering Mandate
...
**Objective:** Combine CAE semantic evaluation with CMF heritage scoring and clustering to form a portfolio of distinct editorial opportunities.
...
- `M07`\n- `OLD CMF intelligence/03_scoring_frameworks`\n- `viral_scoring`\n- `frame alignment`\n- `viral trinity`\n- `current evaluation cards`\n- `vector retrieval infrastructure`\n- `anti-centroid doctrine.`\n
...
Create CandidateEvaluationProfile, scoring adapters, cluster/redundancy detection, coverage analysis, and score provenance. Keep component scores separable and non-compensable gates explicit.
...
No single “virality score” may override semantic truth or evidence. No cluster may be called a quality judgment. No vector similarity may be treated as proof of meaning.
...
The system must distinguish quality, distinctiveness, distribution potential, and confidence. Heritage scorers may contribute evidence but must remain versioned and auditable.
...
Reward-hack cases: score inflation by repeating one source, candidate length gaming, cluster count gaming, or high predicted virality with low evidence. Include human taste corpus where available.
...
**Operator decision:** Approve M08 and authorize M09.
...
```text
Execute CAE-M08 — Scoring and Clustering. Read the mandate and all referenced scoring systems. Build a bounded evaluation layer that scores candidates across separable dimensions such as semantic strength, guest authenticity, audience relevance, novelty, narrative utility, visual opportunity, editorial completeness, and distribution potential. Adapt the existing OLD CMF scoring frameworks instead of discarding them or treating them as unquestioned truth. Preserve score lineage, evaluator version, and evidence inputs. Use clustering to reveal redundancy and coverage, not to declare quality. Vector similarity is retrieval support, not semantic proof. Build anti-reward-hacking tests for repeated-source inflation, length gaming, keyword stuffing, duplicate candidate proliferation, and high predicted virality without evidence. Introduce no automatic production approval. The output is an editorial board with score transparency and cluster relationships. Update control state, commit only scoped changes, request approval of M08 and M09 authorization, then stop. Preserve the distinction between facts, hypotheses, and decisions throughout the work. Any inherited artifact that disagrees with the target architecture must be documented as a collision with evidence rather than silently rewritten. For every test, record what property it genuinely proves and what remains outside its proof boundary. For content-facing work, include a concrete anti-centroid or false-proof case so a structurally green result cannot be mistaken for taste, authenticity, novelty, or narrative completeness. Do not weaken a validator to make a failing implementation pass. Do not begin the next mandate, even if its work appears obvious.
```
