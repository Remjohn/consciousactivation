# CAE Mandate CA-M054 — Unified Telemetry and Governed Human-Resolution Training Flywheel

**Status:** Proposed governed execution mandate for Wave 07.  
**Canonical decision:** Q53  
**Mandate ID:** `CA-M054`  
**Governing invariant:** `INV-TELEM-001`  
**Primary implementation surface:** `packages/ca_runtime/src/ca_runtime/factory_observability.py; packages/ca_runtime/src/ca_runtime/program_operator_runtime.py (telemetry and gate-decision capture)`
...
The canonical decision is Q53; the execution identifier is CA-M054. The primary invariant is `INV-TELEM-001`. This mandate consumes earlier decisions but does not reopen them. Any apparent collision with an earlier invariant must be treated as an authority conflict and escalated through the protocol’s collision procedure rather than resolved by convenience.

## 2. Decision / objective being authorized

The authorized objective is:

> **Capture runtime telemetry in a six-class taxonomy and transform eligible operator gate decisions into PII-redacted, contrastive HumanResolutionEpisode preference pairs without allowing training artifacts to acquire runtime authority.**
...
**Objective:** Capture runtime telemetry in a six-class taxonomy and transform eligible operator gate decisions into PII-redacted, contrastive HumanResolutionEpisode preference pairs without allowing training artifacts to acquire runtime authority.

**Dependencies:** Q24–Q27 authorization and receipts; Q40 human gate milestones; Q50 economics; Q52 CSEB; downstream governed training workflows.

**Allowed runtime/code surface:** `packages/ca_runtime/src/ca_runtime/factory_observability.py; packages/ca_runtime/src/ca_runtime/program_operator_runtime.py (telemetry and gate-decision capture)`
...
**Outputs:** six-class event schema, event emission adapters, HumanResolutionEpisode records, chosen/rejected preference pairs, PII-redaction verifier, training export manifest, audit tests.
...
For this mandate, the principal surface is `packages/ca_runtime/src/ca_runtime/factory_observability.py; packages/ca_runtime/src/ca_runtime/program_operator_runtime.py (telemetry and gate-decision capture)`. The intended artifact class is `six-class event schema, event emission adapters, HumanResolutionEpisode records, chosen/rejected preference pairs, PII-redaction verifier, training export manifest, audit tests.`. No change to unrelated UI surfaces, upstream semantic definitions, external integrations, deployment topology, or later-wave/question contracts is authorized unless a required dependency inside the declared surface makes the change unavoidable. In that case, stop and request an Operator collision decision rather than broadening scope unilaterally.
...
The mandate explicitly prohibits the following: Do not treat telemetry as canonical runtime state, export raw secrets or unrestricted PII, synthesize operator choices that never occurred, use preference data as a direct routing override, or mutate historical telemetry for convenience.
...
For this mandate, the specific required behavior is: Define and validate the six event classes; emit deterministic events at declared lifecycle points; capture genuine operator choices with chosen/rejected context; redact protected data before export; version the export; prove training corpora remain read-only derivations.

The executor must include a contrastive negative test for the following false-proof case:

> A training set contains many plausible preference pairs and produces better offline metrics, but the pairs were generated by the model rather than by real operator gates and include unredacted identifiers. It is statistically attractive but not an authentic governed flywheel.
...
Execute CA-M054 (Q53) only. Read the authoring protocol, Gemini skill, FR-053, convergence decision ledger, Architecture.md, UI.md, readiness assessment, and relevant authorization/gate mandates. Objective: implement the six-class telemetry taxonomy and turn genuine operator gate decisions into PII-redacted HumanResolutionEpisode chosen/rejected preference pairs without granting training artifacts runtime authority. Work only in factory_observability.py, program_operator_runtime.py, directly required telemetry/export schemas/tests, and evidence artifacts. Meaning authority is the Q53 contract; runtime authority remains canonical state; telemetry and training outputs are immutable derivatives; the Operator alone approves promotion. Do not export unrestricted PII or secrets, invent operator choices, mutate historical telemetry, or use preference records as routing overrides. Emit deterministic events at declared lifecycle points, capture authentic chosen/rejected context, redact protected data, version exports, and verify downstream corpora remain read-only. Include the false-proof case where an attractive preference dataset is model-generated rather than based on real Operator resolutions and still contains identifiers. Record evidence classes, redaction tests, schema checks, limitations, control-state update, and commit SHA. Stop on missing privacy controls, authority conflict, undeclared boundary, or any request to train or deploy beyond this mandate. Request Operator decision: APPROVE, REJECT with findings, or DEFER with named remediation. Do not execute Q57.
