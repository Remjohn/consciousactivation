# M27 — Guest Genesis + Protected/Dervied Semantic Territory
...
Activate the existing Guest/Brand persistence surfaces with real reasoning/generation for Voice DNA, Visual DNA, Distillation and semantic territory while preserving authenticated Guest truth and anti-centroid integrity.

## 2. Mandatory reading before action
Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
Mandate-specific:
CURRENT.md F30/§1.3a; `services/air/src/.../brand_service.py`; relevant AIR schemas/adapter; Interview Composer voice-ref read path; Guest constitutions; Skill constitution.
...
## 5. Allowed work
Implement the smallest real generation path, using Agent/Skills where doctrine requires reasoning. Separate protected evidence/memory from mutable derived expressions and store provenance.
...
## 8. Acceptance
A controlled real Guest corpus produces reviewable Voice/Visual/semantic-territory artifacts with provenance; unsupported claims and centroid-collapse are rejected; the existing Composer voice-ref read no longer points only to an unreachable write path.
