# M29 — Research Knowledge Extraction + Canonicalization + OKF
...
Make canonicalization a real multi-agent reasoning Program that transforms raw sources into curated, OKF-compatible canonical knowledge while preserving source evidence.

## 2. Mandatory reading before action
Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
Mandate-specific:
OKF SPEC; Cole SCHEMA/index; Karpathy gist; StateM where stateful; M28; CAE research authorities; `20_PHASE3_CANONICALIZATION_MODEL.md`.
...
## 5. Allowed work
Implement source→knowledge candidate→canonicalization→canonical node/relationship. Use bounded Hunter/Analyst/Composer/Commander roles. Support merge/split/retract/reclassify/re-express semantics where existing authority permits.
...
## 8. Acceptance
A controlled corpus with aliases/duplicates/contradictions yields stable canonical nodes, explicit provenance, false-merge rejection, human/Commander adjudication where required, and an exportable OKF bundle.
