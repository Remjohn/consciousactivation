# M22 — Skill Loader + Maturity + Context Resolution

    Status: PROPOSED — OPERATOR-RATIFICATION-REQUIRED
    Phase: 2 — Runtime Foundation

    ## 1. Decision / Objective
    Make Canonical Skills executable JIT while preserving passive, versioned, flat composition and maturity gates.

    ## 2. Mandatory reading before action
    Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
    Mandate-specific:
    Skill Constitution; current skill packages; evaluators; M18; 22_PHASE2_AGENT_PACKAGE_COMPILATION_CONTRACT.md; existing AGENTS/CAE.md conventions.
...
- Agent text, UI status or a receipt without matching state/artifact evidence is not completion proof.
    ...
    ## 3. Allowed work
    Implement Skill resolution, metadata/trigger loading, hash/version pinning, maturity gating and package-local context. Do not alter SKILL.md into an autonomous actor.
    ...
    Unapproved Skills cannot execute; approved Skills load reproducibly; Skill→Skill invocation is impossible through the runtime API.
