# M21 — Four-Lane Agent Team + Sub-agent Runtime

    Status: PROPOSED — OPERATOR-RATIFICATION-REQUIRED
    Phase: 2 — Runtime Foundation

    ## 1. Decision / Objective
    Implement bounded multi-agent execution while preserving Hunter/Analyst/Composer/Commander as separate authorities and capability projections.

    ## 2. Mandatory reading before action
    Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
    Mandate-specific:
    Phase 1 M9; Skill Constitution; WorkflowRole; 21_PHASE2_CAPABILITY_SECURITY_MATRIX.md; 25_PHASE2_OPERATOR_GATE_RUNTIME_CONTRACT.md; Pi lane/runtime docs.
...
- Agent text, UI status or a receipt without matching state/artifact evidence is not completion proof.
    ...
    ## 3. Allowed work
    Implement Agent/Team/Sub-agent composition, lane binding, bounded concurrency, timeout/cancellation, retry, failure propagation and structured delegation results.
    ...
    A pilot runs at least two lanes and one Sub-agent; wrong-lane work, unauthorized capability and Skill nesting are rejected; provenance is retained.
