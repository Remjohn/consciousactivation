# M23 — Hooks + Extensions + Capability Enforcement Runtime

    Status: PROPOSED — OPERATOR-RATIFICATION-REQUIRED
    Phase: 2 — Runtime Foundation

    ## 1. Decision / Objective
    Turn deterministic CAE guarantees into executable hooks/extensions and capability gates over the Pi runtime.

    ## 2. Mandatory reading before action
    Baseline: `00_CONTROL/01_BASELINE_AUTHORITY_READ_SET.md`
    Mandate-specific:
    Cole hooks-create SKILL; Pi hooks/telemetry/extensions; Eve hooks; Phase 1 M6; 21_PHASE2_CAPABILITY_SECURITY_MATRIX.md; 23_PHASE2_EVENT_TRACE_CONTRACT.md; 25_PHASE2_OPERATOR_GATE_RUNTIME_CONTRACT.md.
...
- Agent text, UI status or a receipt without matching state/artifact evidence is not completion proof.
    ...
    ## 3. Allowed work
    Implement representative pre-tool, post-mutation, state-transfer, completion and recovery hooks; enforce capability allow/deny; capture hook outcomes and recovery paths.
    ...
    Critical guarantees are deterministic; unsafe calls are blocked; completion is impossible without required evidence/gates; hook decisions appear in trace.
