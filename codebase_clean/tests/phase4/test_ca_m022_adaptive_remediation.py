...
def test_session_fails_when_omission_rate_exceeds_maximum() -> None:
    # Isolate the omission ceiling: lower yield threshold so the holistic-yield
    # gate passes while omission (3/4 = 0.75) still exceeds the default ceiling.
    policy = YieldSufficiencyPolicy(
        min_holistic_yield_ratio=0.20,
        min_tension_score=0.40,
        min_evidence_count=1,
        max_omission_rate=0.50,
    )
    units = [
    ...
    assert receipt.gates["holistic_yield_ratio"].passed is True
