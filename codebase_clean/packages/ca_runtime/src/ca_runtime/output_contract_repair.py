...
def greedy_extract_json(raw_text: str) -> str:
    ...
    for candidate in candidates:
        ...
        # Greedy object then array that parses cleanly
    ...
    # Fallback: return the first structural JSON candidate so deterministic_local_repair can fix it
    for candidate in candidates:
        cand_strip = candidate.strip()
        if (cand_strip.startswith("{") and cand_strip.endswith("}")) or (
            cand_strip.startswith("[") and cand_strip.endswith("]")
        ):
            return cand_strip
...
def enforce_output_contract(
    ...
    *,
    ...
    saved_first_exc: Optional[Exception] = None
    ...
    if model_repair_fn is None or max_model_repair_turns < 1:
        ...
        if isinstance(saved_first_exc, JSONExtractionError):
            raise saved_first_exc
        raise SchemaComplianceError(
            f"Schema compliance failed and no model repair available: {saved_first_exc}",
            validation_errors=first_errors,
            raw_text=current_raw,
            repair_attempted=False,
        ) from saved_first_exc
