...
_PRODUCTION_REGISTERED = False
_PHASE9_REGISTERED = False

def _ensure_production_registered() -> None:
    global _PRODUCTION_REGISTERED
    if _PRODUCTION_REGISTERED:
        return
    from .production_domain import PRODUCTION_ID_FIELDS, PRODUCTION_VALIDATORS
    _VALIDATORS.update(PRODUCTION_VALIDATORS)
    _ID_FIELDS.update(PRODUCTION_ID_FIELDS)
    _PRODUCTION_REGISTERED = True
    _ensure_phase9_registered()


def _ensure_phase9_registered() -> None:
    global _PHASE9_REGISTERED
    if _PHASE9_REGISTERED:
        return
    from .phase9_domain import PHASE9_ID_FIELDS, PHASE9_VALIDATORS
    _VALIDATORS.update(PHASE9_VALIDATORS)
    _ID_FIELDS.update(PHASE9_ID_FIELDS)
    _PHASE9_REGISTERED = True



def supported_object_types() -> tuple[str, ...]:
    _ensure_production_registered()
    _ensure_phase9_registered()
...
def schema_registry() -> dict[str, Any]:
    ...
    for object_type, id_field in sorted(_ID_FIELDS.items()):
        result[object_type] = {
            ...
            "$id": f"https://contracts.conscious-activations.local/air-semantic-production/0.3.0-dev.1/{object_type}.schema.json",
            "title": object_type,
            "type": "object",
