...
class AirRepository:
    ...
    def initialize(self, *, now: str | None = None) -> dict[str, Any]:
        ...
        production_migration = resources.files(
            "cmf_activative_intelligence.migrations"
        ).joinpath("0003_semantic_production_compiler.sql").read_text(encoding="utf-8")
        ...
        with closing(self._connect()) as connection:
            ...
            if metadata is None:
                ...
                if observed_version not in {PHASE1_VERSION, PHASE2_VERSION, PRODUCT_VERSION}:
                ...
                if int(metadata["production_authorized"]) != 0 or int(metadata["certified"]) != 0:
                    raise AirRepositoryError(
                        "AIR development runtime cannot open a database claiming production or certification"
                    )
                if observed_version in {PHASE1_VERSION, PHASE2_VERSION}:
                    connection.execute(
                        """
            ...
            connection.executescript(production_migration)
            connection.execute(
                """
                INSERT INTO air_product_migrations(version, name, applied_at_utc)
                VALUES(3, '0003_semantic_production_compiler', ?)
                ON CONFLICT(version) DO NOTHING
                """,
                (timestamp,),
            )
    ...
    def add_object_edge(
        ...
        *,
        ...
        if not relation_type or not isinstance(relation_type, str):
            raise AirRepositoryError("relation_type must be a non-empty string")
        source = self.get_object(str(source_ref["object_id"]))
        if source.immutable_ref() != dict(source_ref):
            raise AirRepositoryError("source_ref does not identify current bytes")
        target_revision = None
        try:
            target = self.get_object(str(target_ref["object_id"]))
            if target.immutable_ref() != dict(target_ref):
                raise AirRepositoryError("target_ref does not identify current bytes")
        ...
        with closing(self._connect()) as connection:
            connection.execute(
                ...
                INSERT INTO air_object_edges(
                    source_object_id, source_revision, relation_type,
                    target_object_id, target_revision, evidence_json, created_at_utc
                ) VALUES(?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(source_object_id, source_revision, relation_type, target_object_id, target_revision)
                DO UPDATE SET evidence_json = excluded.evidence_json, created_at_utc = excluded.created_at_utc
                """,
                (source.object_id, source.revision, relation_type, str(target_ref["object_id"]), target_revision, canonical_json_text(evidence), timestamp),
            )
        return {
            "source_ref": source.immutable_ref(),
            "relation_type": relation_type,
            "target_ref": dict(target_ref),
            "evidence_sha256": canonical_sha256(evidence),
        }

    def list_edges(self, object_id: str, *, outgoing: bool = True) -> tuple[dict[str, Any], ...]:
        self.initialize()
        field = "source_object_id" if outgoing else "target_object_id"
        with closing(self._connect()) as connection:
            rows = connection.execute(
                f"SELECT * FROM air_object_edges WHERE {field} = ? ORDER BY relation_type, source_object_id, target_object_id",
                (object_id,),
            ).fetchall()
        return tuple({
