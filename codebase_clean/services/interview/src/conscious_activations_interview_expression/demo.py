...
def run_demo(database_path: str|Path) -> dict[str,Any]:
    ...
    span=make_source_span(source_ref=package_ref,start_ms=1500,end_ms=3240,speaker_id="guest")
    ...
    quote_text="Um then I learned to listen."
    quote_start=raw_transcript.index(quote_text)
    quote_end=quote_start+len(quote_text)
    evidence=app.verbatim.admit(
        source_package_ref=package_ref,
        alignment_ref=alignment_ref,
        phrase_refs=[phrase_item_ref],
        ...
        character_start=quote_start,
        character_end=quote_end,
        quote_text=quote_text,
        actor_id="demo-extractor",
        limitations=["SYNTHETIC_DEVELOPMENT_FIXTURE"],
        idempotency_key="demo:verbatim",
    )
    inventory=app.inventory.compile(source_package_ref=package_ref,expression_moment_refs=[moment_ref],phrase_pack_ref=phrase_ref,visual_index_ref=visual_ref,reaction_receipt_refs=[reaction_ref],idempotency_key="demo:inventory",verbatim_evidence_refs=[_ref(evidence)])
