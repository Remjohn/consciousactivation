...
title: Batch Dependency Graph and Selective Invalidation
...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-005.md
controlling_frs: ['AIR-FR-110', 'FR-138']
controlling_stories: ['AIR-ST-19.01', 'ST-03.05']
upstream_draft_dependencies: ['TS-AHP-004', 'TS-AIR-019']
---

# TS-AHP-005 - Batch Dependency Graph and Selective Invalidation
...
|---|---|---|---|---|---|
| `SDE-076` | `TS-AHP-004` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-004.md` | `WRITTEN_PENDING_AUDIT` | `771cbdcdc33a6ed7fe175a037740a89181e7c51baa6591ec35c68a9791a24d08` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-077` | `TS-AIR-019` | `04_ACTIVATIVE_INTELLIGENCE_RUNTIME/docs/tech-specs/TS-AIR-019.md` | `WRITTEN_PENDING_AUDIT` | `515e42a7e015c212f9f972b4e78e1e7aa0558816448f10ff18db9b9a7f7ecd5e` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Batch Dependency Graph and Selective Invalidation` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `AIR-FR-110` | `Atomic Harness Pipeline` | `TS-AHP-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-138` | `Atomic Harness Pipeline` | `TS-AHP-005` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `AIR-ST-19.01` | `TS-AHP-005` | `WRITTEN_PENDING_AUDIT` |
| `ST-03.05` | `TS-AHP-005` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_AHP_005_RuntimeContract`. It consumes TS-AHP-004, TS-AIR-019, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_AHP_005_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_AHP_005_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_AHP_005_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_AHP_005_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
