...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-SPV-001.md
controlling_frs: ['FR-061', 'FR-062', 'FR-063', 'FR-064', 'FR-065', 'FR-066', 'FR-146']
controlling_stories: ['ST-05.03']
upstream_draft_dependencies: ['TS-STA-001', 'TS-AHP-003']
---

# TS-SPV-001 - Source-Grounded SuperVisual Runtime and Export Package

This is an implementation-grade candidate Tech Spec for the source-grounded SuperVisual runtime, export and evidence boundary. It was written under the Prompt 02C specification-work authorization and ends at `WRITTEN_PENDING_AUDIT`. It does not authorize implementation, product adoption, build, release bytes, production use, certification, VAE Stage 5, or a Development Capsule.
...
|---|---|---|---|---|---|
| `SDE-072` | `TS-STA-001` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-STA-001.md` | `WRITTEN_PENDING_AUDIT` | `423eee94e20ab263fbbc1d10fefd4a687e823491c3eba1ece66acfcf0302e160` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-073` | `TS-AHP-003` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-AHP-003.md` | `WRITTEN_PENDING_AUDIT` | `072041914b836be5a45e80ee87102cc490f0927a946633e78806bee63e3578ed` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Source-Grounded SuperVisual Runtime and Export Package` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.

The solution scope is the source-grounded SuperVisual runtime, export and evidence boundary. The spec defines required inputs, state transitions, output artifacts, validation receipts, failure handling, migration behavior, rollback boundaries and test obligations. It excludes implementation source code, provider credentials, generated types, schemas, release packages, production signing and certification.
...
|---|---|---|---|
| `FR-061` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-062` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-063` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-064` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-065` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-066` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-146` | `Atomic Harness Pipeline` | `TS-SPV-001` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-05.03` | `TS-SPV-001` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_SPV_001_RuntimeContract`. It consumes TS-STA-001, TS-AHP-003, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_SPV_001_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_SPV_001_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_SPV_001_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_SPV_001_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
