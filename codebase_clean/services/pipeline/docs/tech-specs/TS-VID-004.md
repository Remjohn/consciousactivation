...
output_path: 05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-004.md
controlling_frs: ['FR-071', 'FR-072']
controlling_stories: ['ST-04.04']
upstream_draft_dependencies: ['TS-VID-002', 'TS-VID-003']
---
...
|---|---|---|---|---|---|
| `SDE-078` | `TS-VID-002` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-002.md` | `WRITTEN_PENDING_AUDIT` | `ea8f83e732ac9da4ff8c31816ba9ad7e6c6a8bf866b1b6a5ae4c3bb4c9bc9b13` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
| `SDE-079` | `TS-VID-003` | `05_ATOMIC_HARNESS_PIPELINE/docs/tech-specs/TS-VID-003.md` | `WRITTEN_PENDING_AUDIT` | `352a41e925897233636021ae0fef947019d57c30b2279f09903a53ed198d796c` | `DRAFT_DEPENDENCY_NOT_ACCEPTED` |
...
The problem is that `Temporal Embodiment Binding and FFmpeg Production-Correctness` must be implemented later without collapsing semantic authority, source provenance, category/profile support, evaluation evidence or lifecycle state into informal notes. The user outcome is a replayable and reviewable product boundary where every command, output, denial and repair points back to exact source, owner, version and receipt evidence.
...
|---|---|---|---|
| `FR-071` | `Atomic Harness Pipeline` | `TS-VID-004` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |
| `FR-072` | `Atomic Harness Pipeline` | `TS-VID-004` | `SPECIFICATION_ONLY_NO_IMPLEMENTATION_NO_PRODUCTION_NO_CERTIFICATION` |

| Story | Spec binding | Current quality state |
|---|---|---|
| `ST-04.04` | `TS-VID-004` | `WRITTEN_PENDING_AUDIT` |
...
The central implementation object is `TS_VID_004_RuntimeContract`. It consumes TS-VID-002, TS-VID-003, the Prompt 02C packet, the authority-stage decision and the writing-wave lock. It emits one of three outcomes: a valid artifact package, a typed denial, or a scoped repair/escalation request.
...
|---|---|---|
| `TS_VID_004_Command` | `command_id`, `idempotency_key`, `caller`, `target_version`, `source_refs`, `authority_refs`, `operation` | Reject before side effects when authority or version is stale. |
| `TS_VID_004_State` | `state_id`, `version`, `lifecycle_state`, `pinned_dependencies`, `not_applicable_fields`, `lineage` | Immutable per version and deterministically serialized. |
| `TS_VID_004_Receipt` | `receipt_id`, `command_id`, `result`, `artifact_refs`, `hashes`, `failure_context` | Receipt storage without artifact or denial context is invalid. |
| `TS_VID_004_Invalidation` | `reason`, `owner`, `affected_descendants`, `replacement_refs`, `historical_replay_policy` | Descendant-only unless upstream authority issues a new demand/version. |
