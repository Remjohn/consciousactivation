...
class VideoEditProgramService:
    ...
    def compile(
        ...
        *,
        ...
        refs = {
            field: require_ref(request[field], field)
            for field in (
                "derivative_job_ref",
                "source_registration_ref",
                "semantic_production_package_ref",
        ...
        source_integrity = self.source_media.verify_source(
            source_registration_ref=refs["source_registration_ref"],
            source_path=source_path,
        )
        if source_integrity["source_authority"] != SOURCE_MEDIA_AUTHORITY:
            raise PipelineSourceLineageError("program source authority is not sovereign media bytes")
        canvas = _strict(
            request["canvas"],
            {"width", "height", "fps_numerator", "fps_denominator", "duration_ms"},
            "canvas",
        )
        normalized_canvas = {
        ...
        normalized_timebase = {
            "numerator": require_int(timebase["numerator"], "timebase.numerator", minimum=1),
            "denominator": require_int(timebase["denominator"], "timebase.denominator", minimum=1),
        }
        tracks = []
        primary_count = 0
        element_ids = set()
        for ti, raw_track in enumerate(request["tracks"]):
            ...
            for ei, raw in enumerate(track["elements"]):
                ...
                if element_id in element_ids:
                    raise PipelineValidationError("element IDs must be unique")
                element_ids.add(element_id)
                kind = require_string(elem["kind"], "kind")
                if kind not in ELEMENT_KINDS:
                    raise PipelineValidationError("unsupported element kind")
                out_start = require_int(elem["output_start_ms"], "output_start_ms")
                out_end = require_int(elem["output_end_ms"], "output_end_ms", minimum=1)
                if out_end <= out_start or out_end > normalized_canvas["duration_ms"]:
                    raise PipelineValidationError("invalid output interval")
                source_ref = (
                    "NOT_APPLICABLE"
                    if elem["source_registration_ref"] == "NOT_APPLICABLE"
                    else require_ref(elem["source_registration_ref"], "source_registration_ref")
                )
                source_start = elem["source_start_ms"]
                source_end = elem["source_end_ms"]
                if kind == "SOURCE_SEGMENT":
                    if source_ref == "NOT_APPLICABLE":
                        raise PipelineValidationError("source segment requires source registration")
                    if source_ref != refs["source_registration_ref"]:
                        raise PipelineSourceLineageError(
                            "source segment registration does not match the program sovereign source registration"
                        )
                    source_start = require_int(source_start, "source_start_ms")
                    source_end = require_int(source_end, "source_end_ms", minimum=1)
                    if source_end <= source_start:
                        raise PipelineValidationError("invalid source interval")
                else:
                    if source_start != "NOT_APPLICABLE" or source_end != "NOT_APPLICABLE":
                        raise PipelineValidationError("non-source element source times must be NOT_APPLICABLE")
                artifact_ref = (
                    "NOT_APPLICABLE"
                    if elem["artifact_ref"] == "NOT_APPLICABLE"
                    else require_ref(elem["artifact_ref"], "artifact_ref")
                )
                bbox_ref = (
                    "NOT_APPLICABLE"
                    if elem["bbox_intent_ref"] == "NOT_APPLICABLE"
                    else require_ref(elem["bbox_intent_ref"], "bbox_intent_ref")
                )
                generated_state = require_string(elem["generated_slot_state"], "generated_slot_state")
                if kind == "GENERATED_SLOT" and generated_state not in {"UNMATERIALIZED", "MATERIALIZED"}:
                    raise PipelineValidationError("invalid generated slot state")
                if kind == "GENERATED_SLOT" and generated_state == "UNMATERIALIZED" and artifact_ref != "NOT_APPLICABLE":
                    raise PipelineValidationError("unmaterialized generated slot cannot have artifact")
                elements.append(
                    {
                        ...
                        "semantic_role": require_string(elem["semantic_role"], "semantic_role"),
                        "sequence_role": require_string(elem["sequence_role"], "sequence_role"),
                        "source_registration_ref": source_ref,
                        "source_start_ms": source_start,
                        "source_end_ms": source_end,
                        "artifact_ref": artifact_ref,
                        "generated_slot_state": generated_state,
                        "bbox_intent_ref": bbox_ref,
                        "text": require_string(elem["text"], "text")
                        if elem["text"] != "NOT_APPLICABLE"
                        else "NOT_APPLICABLE",
                    }
                )
            ...
            tracks.append(
                {
                    "track_id": require_string(track["track_id"], "track_id"),
                    "track_type": track_type,
                    "role": role,
                    "z_index": require_int(track["z_index"], "z_index"),
                    "elements": elements,
                }
            )
        if primary_count != 1:
            raise PipelineValidationError("source-led program requires exactly one PRIMARY_A_ROLL_SPINE")
        ...
        self.repository.add_edge(
            source_integrity["source_media_ref"]["object_id"],
            program["program_id"],
    ...
    def projection(self, program_id: str) -> dict[str, Any]:
        ...
        return {
            "program_ref": {
                ...
                "version": payload["program_version"],
                "sha256": obj["canonical_sha256"],
            },
