...
|---|---|---|
| Contract/validator suite | `python -m unittest discover -s packages/validators/tests -v` | 61 passed |
| Reference protocol suite | `python -m pytest packages/protocol/tests -q -p no:cacheprovider` | 33 passed |
...
release packaging. RC2 preserves RC1 as historical evidence, separates the
source-only manifest, generates the canonical manifest from staged release
contents, and makes every shipped validator release-root relative.
