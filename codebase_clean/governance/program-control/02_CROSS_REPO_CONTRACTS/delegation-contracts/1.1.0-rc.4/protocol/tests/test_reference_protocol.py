...
from cmf_delegation_validators.paths import FIXTURES_ROOT
...
def test_source_provenance_is_enforced_without_protocol_inference() -> None:
    invalid_demands = []
    missing = payload("visual-asset-demand")
    missing.pop("source_provenance")
    invalid_demands.append(missing)
    unknown = payload("visual-asset-demand")
    unknown["source_provenance"]["source_kind"] = "untyped_guess"
    invalid_demands.append(unknown)
    for field in ("reaction_receipt_refs", "expression_moment_refs"):
        omitted = payload("visual-asset-demand")
        omitted["activative_semantic_lineage"].pop(field)
        invalid_demands.append(omitted)
        empty = payload("visual-asset-demand")
        empty["activative_semantic_lineage"][field] = []
        invalid_demands.append(empty)
    harness = Harness()
    for index, invalid in enumerate(invalid_demands, start=1):
        rejected, _ = harness.send(
            "visual-asset-demand",
            invalid,
            "CONTENT_HARNESS",
            f"corr-source-provenance-invalid-{index}",
        )
        assert rejected.code == "PAYLOAD_SCHEMA_INVALID"
        assert "source_provenance" not in invalid or invalid["source_provenance"] != {
            "source_kind": "operator_supplied"
        }


def test_non_interview_source_allows_absent_interview_refs_and_preserves_supplied_kind() -> None:
    harness = Harness()
    body = payload("visual-asset-demand")
    body["source_provenance"]["source_kind"] = "authored_source"
    body["activative_semantic_lineage"].pop("reaction_receipt_refs")
    body["activative_semantic_lineage"].pop("expression_moment_refs")
    before = deepcopy(body)
    result, _ = harness.send(
        "visual-asset-demand", body, "CONTENT_HARNESS", "corr-authored-source"
    )
    assert result.accepted
    assert body == before
    assert body["source_provenance"]["source_kind"] == "authored_source"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("source-provenance-parse-only.invalid.json", "SEMANTIC_ENFORCEMENT_UNSUPPORTED"),
    ],
)
def test_behavioral_compatibility_gaps_fail_before_admission(
    ...
    harness = Harness()
    fixture_path = FIXTURES_ROOT / "compatibility" / "constitutional" / fixture_name
...
def test_protocol_accepts_and_preserves_valid_derivative_lock_relationship() -> None:
    harness = Harness()
    body = payload("derivative-lock-inheritance")
    before = deepcopy(body)
    result, _ = harness.send(
        "derivative-lock-inheritance",
        body,
        "VISUAL_ASSET_EDITOR",
        "corr-derivative-lock-valid",
    )
    assert result.accepted
    assert result.state == "DRAFT"
    assert body == before
    assert (
        harness.store.audit("corr-derivative-lock-valid")[0].receipt["payload_hash"]
        == canonical_hash(before)
    )


def test_protocol_rejects_weakened_derivative_lock_without_lifecycle_effect() -> None:
    harness = Harness()
    body = payload("derivative-lock-inheritance")
    body["derivative_wrong_reading_locks"][0]["enforcement_level"] -= 1
    result, _ = harness.send(
        "derivative-lock-inheritance",
        body,
        "VISUAL_ASSET_EDITOR",
        "corr-derivative-lock-weakened",
    )
    assert result.code == "PARENT_LOCK_WEAKENED"
    assert result.state == "DRAFT"
    assert harness.store.state("corr-derivative-lock-weakened") == "DRAFT"
