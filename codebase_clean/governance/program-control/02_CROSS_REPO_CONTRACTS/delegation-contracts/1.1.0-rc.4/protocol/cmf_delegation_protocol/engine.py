...
class ProtocolEngine:
    """Deterministic reference interpreter over the 1.1.0-rc.4 contract package."""
    ...
    def _apply_message(
        ...
        if message_type == "derivative-lock-inheritance":
            outcome = validate_derivative_lock_inheritance(payload)
            if not outcome.get("valid"):
                raise ProtocolRejection(
                    str(outcome["status"]),
                    "derivative_lock_validation",
                    str(outcome.get("reason") or outcome.get("affected_lock_ids") or "invalid"),
                )
            return None, None
