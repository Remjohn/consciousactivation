...
class ReleaseIdentityTests(unittest.TestCase):
    def test_all_current_package_declarations_resolve_to_rc4(self) -> None:
    ...
    def test_active_rc1_rc2_and_rc3_package_declarations_are_rejected(self) -> None:
    ...
    def test_explicit_historical_rejection_evidence_is_allowed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            (root / "RC3_CONVERGENCE_REJECTION_REPORT.md").write_text(
                "historical_evidence: true\nRejected: " + "1.1.0" + "-rc.3\n",
                encoding="utf-8",
            )
            self.assertEqual(scan_stale_package_declarations(root), [])
